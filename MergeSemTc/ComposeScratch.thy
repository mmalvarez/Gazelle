...
definition triv_td ::
  "'a itself \<Rightarrow> ('a, 'a md_triv) ptd'" where
"triv_td _ =
  (mdt, Some o (case_md_triv id))"
...
definition fst_td ::
  "('a, 'b1 :: Mergeableb) ptd' \<Rightarrow>
   ('a, 'b1 * ('b2 :: Mergeableb)) ptd'" where
"fst_td t =
  (case t of
    (ToC, OfC) \<Rightarrow>
...
definition snd_td ::
  "('a, 'b2 :: Mergeableb) ptd' \<Rightarrow>
   ('a, ('b1 :: Mergeableb) * 'b2) ptd'" where
"snd_td t =
  (case t of
    (ToC, OfC) \<Rightarrow>
...
definition prod_td ::
  "('a1, 'b1 :: Mergeableb) ptd' \<Rightarrow>
   ('a2, 'b2 :: Mergeableb) ptd' \<Rightarrow>
...
value "ptd_map
        (option_td (triv_td (TYPE(int))))
        (\<lambda> x . x + 1)
        (Some (mdt (1 :: int)))"

(* something is off here. *)
term "
        (prod_td (option_td (triv_td (TYPE(int)))) (option_td (triv_td (TYPE(int)))))"

value "ptd_map
        (prod_td (option_td (triv_td (TYPE(int)))) (option_td (triv_td (TYPE(int)))))
        (\<lambda> x . case x of (x1, x2) \<Rightarrow> (x1 + 1, x2 + 1))
        (Some (mdt 1), Some (mdt 2))"



(* for this one we need some kind of bsup or projection thing
   because otherwise we will lose the prio data. *)

(*
definition prio_td_keep ::
  "('a1, 'b1 :: Mergeableb) ptd' \<Rightarrow>
   ('a1, 'b1 md_prio) ptd'" where
"prio_td_keep t =
  (case t of
    (ToC, OfC) \<Rightarrow>
  *)    
