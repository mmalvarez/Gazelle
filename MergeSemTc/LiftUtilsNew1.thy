...
record ('syn, 'a, 'b) lifting =
  ...
  LOut2 :: "('syn \<Rightarrow> 'b \<Rightarrow> 'b \<Rightarrow> 'a)"
...
"triv_l t =
  ...
  , LOut1 = (\<lambda> s b . (case b of (mdt b') \<Rightarrow> (LOut1 t s b')))
  , LOut2 = (\<lambda> s b1 b2 . (case b1 of (mdt b1') \<Rightarrow> (case b2 of (mdt b2') \<Rightarrow> (LOut2 t s b1' b2')) ))\<rparr>"
...
"option_l t =
  ...
  , LOut1 = (\<lambda> s b . (case b of Some b' \<Rightarrow> LOut1 t s b'))
  , LOut2 = (\<lambda> s b1 b2 . (case b1 of Some b1' \<Rightarrow> (case b2 of Some b2' \<Rightarrow> LOut2 t s b1' b2')))\<rparr>"
...
"prio_l f0 f1 t =
  ...
  , LOut1 =
      (\<lambda> s p . (case p of
                 mdp m b \<Rightarrow> LOut1 t s b))
  , LOut2 =
      (\<lambda> s p1 p2 . (case p1 of
                  mdp m b1 \<Rightarrow> (case p2 of
                                mdp m b2 \<Rightarrow> LOut2 t s b1 b2)))\<rparr>"
...
"oalist_l tk tv =
  \<lparr> LIn1 = (\<lambda> s a . update (LIn1 tk s a) (LIn1 tv s a) empty)
  , LIn2 = (\<lambda> s a l .
            (let k = LIn1 tk s a in
            (case get l k of
              None \<Rightarrow> update k (LIn1 tv s a) l
              | Some v \<Rightarrow> update k (LIn2 tv s a v) l)))
  , LOut1 = (\<lambda> s l . 
  , LOut2 = undefined \<rparr>"
...
"l_pred_step l P s st1 st2 =
        (P s (LOut1 l s st1) (LOut2 l s st1 st2))"
...
"l_pred_step2' l1 l2 P syn st1 st2 =
   (P (l1 syn) (LOut1 l2 (l1 syn) st1) (LOut2 l2 (l1 syn) st1 st2))"
...
"lifting_valid l =
 ((\<forall> s a .  LOut1 l s (LIn1 l s a) = a) \<and>
  (\<forall> s a b . LOut1 l s (LIn2 l s a b) = a) \<and>
  (\<forall> s a b . LOut2 l s b (LIn1 l s a) = a) \<and>
  (\<forall> s a b b' . LOut2 l s b (LIn2 l s a b') = a))"

lemma lifting_valid_intro :
  assumes H1 : "\<And> s a .  LOut1 l s (LIn1 l s a) = a"
  assumes H2 : "\<And> s a b . LOut1 l s (LIn2 l s a b) = a"
  assumes H3 : "\<And> s a b . LOut2 l s b (LIn1 l s a) = a"
  assumes H4 : "\<And> s a b b' . LOut2 l s b (LIn2 l s a b') = a"
...
lemma lifting_valid_unfold3 :
  assumes H : "lifting_valid l"
  shows "LOut2 l s b (LIn1 l s a) = a"
  using H by (auto simp add:lifting_valid_def)


lemma lifting_valid_unfold4 :
  assumes H : "lifting_valid l"
  shows "LOut2 l s b (LIn2 l s a b') = a"
  using H by (auto simp add:lifting_valid_def)
...
lemma pred_lift2' :
  ...
  using Hsyn Hsem HP lifting_valid_unfold2[OF Hv] lifting_valid_unfold4[OF Hv]
