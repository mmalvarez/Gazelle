...
definition is_bub :: "'a \<Rightarrow> 'a \<Rightarrow> 'b \<Rightarrow> bool" where
"is_bub a b s =
  (pleq a (aug s) \<and>
    ((\<forall> bd sd . pleq bd b \<longrightarrow>
                Pord.is_sup pleq {a, bd} sd \<longrightarrow>
                pleq sd (aug s))))"



definition is_bsup :: "'a \<Rightarrow> 'a \<Rightarrow> 'b \<Rightarrow> bool" where
"is_bsup a b s =
