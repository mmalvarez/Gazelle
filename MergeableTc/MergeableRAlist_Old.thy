theory MergeableRAlist_Old
  imports MergeableAList "HOL-Library.DAList"

begin


typedef (overloaded) ('value) cxt =
  "{ rc :: (unit, 'value) alist . length (DAList.impl_of rc) = 20
      }"
...
qed
*)
value "update (STR ''k'') (mdt 0 :: nat md_triv) empty"
...
lift_bnf (dead 'key :: linorder, set: 'value) cxt [wits: "(update [] (Inr ())) (empty :: ('key :: linorder, 'value) recclos)"]
  sorry
...
proof-  
  ...
  show "rc_valid (recclos_bsup l1 l2)"
  proof(rule rc_valid_intro)
    show "get (recclos_bsup l1 l2) [] = Some (Inr ())"
      apply(transfer)
(*
