...
next
  ...
  assume Supr : "is_sup V supr"
  assume Supr_in : "supr \<in> fst_l_S S1 s \<inter> snd_l_S S2 s"

  obtain res1 res2 where Res: "f s (LOut (fst_l l1) s supr, LOut (snd_l l2) s supr) = (res1, res2)"
    by(fastforce)
  ...
  show "is_sup
        ((\<lambda>x. case f s (LOut (fst_l l1) s x,
                         LOut (snd_l l2) s x) of
               (r1, r2) \<Rightarrow>
                 LUpd (fst_l l1) s r1
                  (LUpd (snd_l l2) s r2 x)) `
         V)
        (case f s (LOut (fst_l l1) s supr,
                   LOut (snd_l l2) s supr) of
         (r1, r2) \<Rightarrow>
           LUpd (fst_l l1) s r1
            (LUpd (snd_l l2) s r2 supr))"
  proof(rule is_supI)
    fix x

    assume X: "x \<in> (\<lambda>x. case f s (LOut (fst_l l1) s x,
LOut (snd_l l2) s x) of
                  (r1, r2) \<Rightarrow>
                    LUpd (fst_l l1) s r1
                     (LUpd (snd_l l2) s r2 x)) `
    ...
    then obtain xo where Xo: "xo \<in> V" "(\<lambda>x. case f s (LOut (fst_l l1) s x,
  LOut (snd_l l2) s x) of
                  (r1, r2) \<Rightarrow>
                    LUpd (fst_l l1) s r1
                     (LUpd (snd_l l2) s r2 x)) xo = x"
      by fastforce
