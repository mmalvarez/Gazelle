...
next
  ...
  have In1 :"supr \<in> S1 s" and In2 : "supr \<in> S2 s" using
      compat_S1[OF Supr] compat_S2[OF Supr]
    by auto
(*
...
next
  ...
  show "LUpd (merge_l l1 l2) s a b \<in> S1 s \<inter> S2 s"
    using compat_S1[OF Supr'] compat_S2[OF Supr'] A
    by(auto simp add: merge_l_def)
...
next
  ...
  fix a1
  fix supr
  assume Sup: "is_sup {LUpd (option_l l1) s a1 b, x2} supr"

  obtain x1' where X1' : "LUpd (option_l l1) s a1 b = Some x1'" "x1' \<in> S1 s" using in1.put_S
    by(cases b; auto simp add: option_l_def)

  show "supr \<in> option_l_S S1 s"
  proof(cases supr)
    case None

    then have "Some x1' <[ None"
      using is_supD1[OF Sup, of "Some x1'"] unfolding X1' None
      by auto
  ...
  next
    ...
    proof(cases x2)
      ...
      have Sup_x1 : "is_sup ({Some x1', x2 }) (Some x1')"
      proof(rule is_supI)
        fix w
        assume "w \<in> {Some x1', x2}"
        then show "w <[ Some x1'"
          using None' X1'
          by(auto simp add: option_pleq leq_refl)
      next

        fix z
        assume Ub : "is_ub {Some x1', x2} z"
        show "Some x1' <[ z" using is_ubE[OF Ub, of "Some x1'"] by auto
      qed

      then have "supr = Some x1'"
        using is_sup_unique[OF Sup_x1 Sup[unfolded X1']]
        by auto

      then show "supr \<in> option_l_S S1 s" using X1'
        by (auto simp add: option_l_S_def)
    next
      ...
      hence Supr'_in : "supr' \<in> S1 s"
        using compat_S1[] Sup_inner X1'
        by(cases b; auto simp add: option_l_def)
...
next
  ...
  fix a2
  fix supr
  assume Sup: "is_sup {x1, LUpd (option_l l2) s a2 b} supr"

  obtain x2' where X2' : "LUpd (option_l l2) s a2 b = Some x2'" "x2' \<in> S2 s"
    using in2.put_S
    by(cases b; auto simp add: option_l_def)

  show "supr \<in> option_l_S S2 s"
  proof(cases supr)
    case None

    then have "Some x2' <[ None"
      using is_supD1[OF Sup] unfolding X2' None
      by auto

    then have False using X2' by(simp add: option_pleq)
  ...
  next
    ...
    proof(cases x1)
      ...
      have Sup_x1 : "is_sup ({x1, Some x2' }) (Some x2')"
      proof(rule is_supI)
        fix w
        assume "w \<in> {x1, Some x2'}"
        then show "w <[ Some x2'"
          using None' X2'
          by(auto simp add: option_pleq leq_refl)
      next
        ...
        fix z
        assume Ub : "is_ub {x1, Some x2'} z"
        show "Some x2' <[ z" using is_ubE[OF Ub, of "Some x2'"] by auto
      qed

      then have "supr = Some x2'"
        using is_sup_unique[OF Sup_x1 Sup[unfolded X2']]
        by auto

      then show "supr \<in> option_l_S S2 s" using X2'
        by (auto simp add: option_l_S_def)
    next
      ...
      hence Supr'_in : "supr' \<in> S2 s"
        using compat_S2[] Sup_inner X2'
        by(cases b; auto simp add: option_l_def)
...
*)
  ...
  have Sup' : "is_sup {x1, y1} s1"
    using X Y S
    using is_sup_fst'[OF _ Sup[unfolded S]]
    by(auto simp add: fst_l_def)

  show "supr \<in> fst_l_S S1 s"
    using compat_S1[OF X(2) Sup'] S
    by(auto simp add: fst_l_S_def)
next
  fix x y :: "('c * 'g)"
  fix s
  ...
  show "supr \<in> fst_l_S S2 s"
    using compat_S2[OF Y(2) Sup'] S
    by(auto simp add: fst_l_S_def)
next
...
next
  ...
  assume Sup : "is_sup {LUpd (fst_l l1) s a1 b, y} supr"
  ...
  have "is_sup {LUpd l1 s a1 b1, y1} supr1"
  proof(rule is_supI)

    fix w
    assume "w \<in> {LUpd l1 s a1 b1, y1}"

    then show "w <[ supr1"
      using is_supD1[OF Sup] is_supD1[OF Sup] B
      unfolding Supr1_2 Y
      by(auto simp add: fst_l_def prod_pleq)
  next

    fix w'
    assume Ub : "is_ub {LUpd l1 s a1 b1, y1} w'"

    have Ub' : "is_ub {LUpd (fst_l l1) s a1 b, y} (w', supr2)"
    proof(rule is_ubI)

      fix z
      assume Zin : "z \<in> {LUpd (fst_l l1) s a1 b, y}"

      obtain x1 x2 where X: "LUpd (fst_l l1) s a1 b = (x1, x2)"
        by(cases "LUpd (fst_l l1) s a1 b", auto simp add: fst_l_def)

      have Leqx2 : "x2 <[ supr2"
        using is_supD1[OF Sup, of "LUpd (fst_l l1) s a1 b"]
        unfolding X Supr1_2
        by(auto simp add: prod_pleq)

      have Leqy2 : "y2 <[ supr2"
        using is_supD1[OF Sup, of y]
        unfolding Y Supr1_2
        by(auto simp add: prod_pleq)

      show "z <[ (w', supr2)"
        using Zin Leqx2 Leqy2
          is_ubE[OF Ub, of x1] is_ubE[OF Ub, of y1] X
        unfolding Y B
        by(auto simp add: prod_pleq leq_refl fst_l_def)
    qed

    show "supr1 <[ w'"
      using is_supD2[OF Sup Ub']
      unfolding Supr1_2
      by(auto simp add: prod_pleq)
  qed

  show " supr \<in> fst_l_S S1 s"
    unfolding fst_l_S_def snd_l_S_def Supr1_2  Y
    apply(auto)
...
(* problem: what if y1 is greater than x1, even after update?
we could have some kind of "closure under increase" property...?
*)
...
next
  ...
  assume X1_2in : "x1_2 \<in> S1 s \<inter> S2 s"
  then have X1_2_in1 : "x1_2 \<in> S1 s" and X1_2_in2 : "x1_2 \<in> S2 s"
    by auto

  assume X3_in : "x3 \<in> S3 s"

  assume Supr : 
    "is_sup {x1_2, x3} supr"

  have Supr_in13 :"supr \<in> S1 s \<inter> S3 s"
    using orth1_3.compat_S[OF X1_2_in1 X3_in Supr]
    by auto

  have Supr_in23 : "supr \<in> S2 s \<inter> S3 s"
    using orth2_3.compat_S[OF X1_2_in2 X3_in Supr]
    by auto

  show "supr \<in> S1 s \<inter> S2 s \<inter> S3 s"
    using Supr_in13 Supr_in23
    by auto
next

  fix b s a1_2 a3 supr

  assume "is_sup
        {LUpd (merge_l l1 l2) s a1_2 b, LUpd l3 s a3 b}
        supr"

  show "LOut (merge_l l1 l2) s supr = a1_2" using orth1_2.compat_get1
