...
datatype ('syn, 'a, 'b, 'f) lifting =
  LMake  (LPut : "('syn \<Rightarrow> 'a \<Rightarrow> 'b \<Rightarrow> 'b)")
...
locale lifting_putonly =
  ...
  assumes put_S : "\<And> s a b . LPut l s a b \<in> S s"
  assumes post_S : "\<And> s b . b \<in> S s \<Longrightarrow> LPost l s b \<in> S s"

lemma (in lifting_putonly) upd_S :
  shows "LUpd l s a b \<in> S s"
  using post_S[OF put_S]
  by auto
...
locale lifting_presonly =
  ...
  assumes post_pres :
    "\<And> v V supr s .
        v \<in> V \<Longrightarrow>
        V \<subseteq> S s \<Longrightarrow>
        is_sup V supr \<Longrightarrow> supr \<in> S s \<Longrightarrow> is_sup (LPost l s ` V) (LPost l s supr)"
...
locale lifting_valid_weak =
  lifting_putonly +
  assumes put_get : "\<And> s a b . LOut l s (LPut l s a b) = a"
  assumes post_get : "\<And> s b . LOut l s (LPost l s b) = LOut l s b"
  assumes post_geq : "\<And> s b . b <[ LPost l s b"
  assumes get_put_weak : "\<And> s b . b \<in> S s \<Longrightarrow> b = LUpd l s (LOut l s b) b"

lemma (in lifting_valid_weak) upd_get :
  shows "LOut l s (LUpd l s a b) = a"
  using put_get post_get
  unfolding LUpd_def
  by auto

(* TODO: is there a simpler way to state this *)
...
lemma (in lifting_valid_weak_pres) pres :
  fixes v supr s
  fixes V :: "'c set"
  assumes Vin : "v \<in> V"
  assumes Vsub : "V \<subseteq> S s"
  assumes Vsup : "is_sup V supr"
  assumes Vsup_in : "supr \<in> S s"
  shows "is_sup (LUpMap l f s ` V) (LUpMap l f s supr)"
proof-

  have Sup1 : "is_sup (LMap l f s ` V) (LMap l f s supr)"
    using put_pres[OF Vin Vsub Vsup Vsup_in, of f]
    by auto

  have Vin' : "LMap l f s v \<in> LMap l f s ` V"
    using imageI[OF Vin]
    by auto

  have Vsub' : "LMap l f s ` V \<subseteq> S s"
  proof
    fix x
    assume X : "x \<in> LMap l f s ` V"

    then obtain xo where Xo : "xo \<in> V" "LMap l f s xo = x"
      by auto
  ...
  have Vsup_in' : "LMap l f s supr \<in> S s"
    unfolding LMap_def
    using put_S
    by auto

  have Sup2 : "is_sup ((LPost l s o LMap l f s) ` V) (LPost l s (LMap l f s supr))"
    using post_pres[OF Vin' Vsub' Sup1 Vsup_in']
    unfolding image_comp
    by auto

  then show "is_sup (LUpMap l f s ` V) (LUpMap l f s supr)"
    unfolding LUpMap_def LUpd_def LMap_def image_comp
    by(auto)
qed
...
assumes compat_put : "\<And> s a1 a2 . LPut l1 s a1 (LPut l2 s a2 b) = LPut l2 s a2 (LPut l1 s a1 b)"
(* YOU ARE HERE 
figure out how to relate the interleaving approach to the sup approach.
interleaving approach is ideal to use as the underlying notion, because
