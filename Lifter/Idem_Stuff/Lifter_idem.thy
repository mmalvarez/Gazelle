...
locale l_ortho =
  ...
  assumes compat_idem : "\<And> s a1 a2 b  .  
    has_sup {LUpd_Idem l1 s a1 b, LUpd_Idem l2 s a2 b}"

  assumes compat_post : "\<And>  s . has_sup {LPost l1 s b, LPost l2 s b}"

  assumes compat_idem_S : "\<And> s a1 a2 b1 b2  supr . 
      is_sup {LUpd_Idem l1 s a1 b1, LUpd_Idem l2 s a2 b2} supr \<Longrightarrow>
                        supr \<in> (S1 s \<inter> S2 s)"

  assumes compat_post_S : "\<And> s b1 b2 supr .
        b1 \<in> S1 s \<Longrightarrow>
        b2 \<in> S2 s \<Longrightarrow>
        is_sup {LPost l1 s b1, LPost l2 s b2} supr \<Longrightarrow>
        supr \<in> (S1 s \<inter> S2 s)"

  assumes compat_idem_get1 : 
      "\<And> s a1 a2 b1 b2 supr .
         is_sup {LUpd_Idem l1 s a1 b1, LUpd_Idem l2 s a2 b2} supr \<Longrightarrow>
                        LOut l1 s supr = a1"
  
    assumes compat_idem_get2 : "\<And> s a1 a2 b1 b2 supr . 
          is_sup {LUpd_Idem l1 s a1 b1, LUpd_Idem l2 s a2 b2} supr \<Longrightarrow>
                        LOut l2 s supr = a2"

  assumes compat_post_get1 : "\<And> s b1 b2 supr . 
     is_sup {LPost l1 s b1, LPost l2 s b2} supr \<Longrightarrow>
      LOut l1 s supr = LOut l1 s b1"
  
  assumes compat_post_get2 : "\<And> s b1 b2 supr . 
    is_sup {LPost l1 s b1, LPost l2 s b2} supr \<Longrightarrow>
        LOut l2 s supr = LOut l2 s b2"
...
locale l_ortho_strong = l_ortho +
  assumes compat_put_put :
    "\<And> s x1 x2 y1 y2 b1 b2 supr1 supr2 supr3.
...
lemma (in l_ortho) compat :
  fixes s a1 a2 b1  supr
  shows "has_sup {LUpd l1 s a1 b, LUpd l2 s a2 b}"
proof-


  show "has_sup {LUpd l1 s a1 b, LUpd l2 s a2 b}"
    using compat_post
    unfolding LUpd_def
    by(auto)
qed
