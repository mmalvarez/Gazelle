...
proof
  ...
  obtain supr2 where Supr2 :
    "is_sup {LPost l1 s supr1, LPost l2 s supr1} supr2"
    using compat_post[OF ]
    by(fastforce simp add: has_sup_def)


  have Eq2 : "[^LPost l1 s supr1, LPost l2 s supr1 ^] = supr2"
  ...
  have GetL1 : "LOut l1 s [^LPost l1 s (LUpd_Idem l1 s a1 b), LPost l2 s (LUpd_Idem l2 s a2 b) ^] = a1"
    using compat_post_get1[OF Supr2] Supr1
    using compat_idem_get1[OF Supr1]
    using Eq1 Eq2
    apply(auto)
...
next
  ...
  proof(cases b1)
    ...
    proof(cases b2)
      ...
      have BB: "is_sup {LBase l1 s, LBase l2 s} (LBase l1 s)"
        using eq_base
        by(auto simp add: is_sup_def is_least_def is_ub_def leq_refl)

      then obtain sup1 where Sup1 : 
          " is_sup {LUpd_Idem (l1) s a1 (LBase l1 s),
          LUpd_Idem (l2) s a2 (LBase l2 s)} sup1"
        using compat_idem[OF BB]
        by(fastforce simp add: has_sup_def)

      then show ?thesis using None None' eq_base compat_idem[OF BB] is_sup_Some[OF _ Sup1]
        by(fastforce simp add: option_l_def has_sup_def)
    next
      case Some' : (Some a)
