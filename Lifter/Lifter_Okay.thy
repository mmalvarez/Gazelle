...
locale lifting_putonly =
  fixes l :: "('syn, 'a :: Okay, 'b :: {Okay,Pord_Weak}, 'f) lifting"
  assumes put_S : "\<And> s a b . a \<in> ok_S \<Longrightarrow> LUpd l s a b \<in> ok_S"

locale lifting_presonly =
  fixes l :: "('syn, 'a, 'b :: {Okay,Pord_Weak}, 'f) lifting"
  assumes pres :
    "\<And> v V supr f s . 
         v \<in> V \<Longrightarrow>
         V \<subseteq> ok_S \<Longrightarrow>
         is_sup V supr \<Longrightarrow> supr \<in> ok_S \<Longrightarrow> is_sup (LMap l f s ` V) (LMap l f s supr)"
...
locale lifting_valid_weak =
  ...
  assumes get_put_weak : "\<And> s b . b \<in> ok_S \<Longrightarrow> b <[ LUpd l s (LOut l s b) b"
...
locale lifting_valid_weak_base' =
  fixes l :: "('syn, 'a, 'b :: {Okay,Pord_Weakb}, 'f) lifting"
  fixes bot_alt :: "'b"
  assumes bot_alt_is_bot : "bot_alt = \<bottom>"

locale lifting_valid_weak_base = lifting_valid_weak_base' + lifting_valid_weak +
...
(* TODO: why is bot_alt needed to avoid extra type var? *)
locale lifting_valid_weak_base_pres = lifting_valid_weak_base + lifting_valid_weak_pres + 
  assumes bot_bad : "(bot_alt) \<notin> ok_S"
...
*)
locale l_ortho' =
  fixes T :: "('c :: {Pord, Okay}) itself"
  fixes l1 :: "('a, 'b1, 'c, 'f1) lifting"
  fixes l2 :: "('a, 'b2, 'c , 'f2) lifting"

locale l_ortho =
  (*fixes T :: "('c :: {Pord, Okay}) itself"*)
  fixes l1 :: "('a, 'b1, 'c :: {Pord, Okay}, 'f1) lifting" 
  fixes l2 :: "('a, 'b2, 'c , 'f2) lifting"
(*
...
(*
  ...
  assumes compat_S2 : "\<And> s x2 a1 supr . x2 \<in> S2 s \<Longrightarrow> is_sup {LUpd l1 s a1 b, x2} supr \<Longrightarrow>
                      supr \<in> (S1 s \<inter> S2 s)"
*)
  assumes compat_S : "\<And> (x1 :: 'c) (x2 :: 'c) (supr :: 'c) . x1 \<in> ok_S\<Longrightarrow> x2 \<in> ok_S \<Longrightarrow>
    is_sup {x1, x2} supr \<Longrightarrow> supr \<in> ok_S"
(* TODO: need these set membership constraints? originally there were some. *)
  assumes compat_get1 : "\<And> s a1 a2 (supr :: 'c) . is_sup {LUpd l1 s a1 b, LUpd l2 s a2 b} supr \<Longrightarrow>
                      LOut l1 s supr = a1"
  assumes compat_get2 : "\<And> s a1 a2 (supr :: 'c) . is_sup {LUpd l1 s a1 b, LUpd l2 s a2 b} supr \<Longrightarrow>
                      LOut l2 s supr = a2"
  ...
  assumes put1_S2 : "\<And> s a (b :: 'c) . LUpd l1 s a b \<in> ok_S"
  assumes put2_S1 : "\<And> s a (b :: 'c) . LUpd l2 s a b \<in> ok_S"
...
next
  ...
  assume X1  :"x1 \<in> ok_S"
  assume X2 : "x2 \<in> ok_S"

  assume "is_sup {x1, x2} supr"

  then show "supr \<in> ok_S"
    using compat_S[OF X1 X2]
    by auto
