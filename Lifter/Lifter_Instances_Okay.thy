theory Lifter_Instances_Okay imports Lifter_Instances
begin

(* ok, this does not work, because LUpd does not guarantee
 * we will be in ok_S.
 * but, maybe that isn't the end of the world, often we know that b (start state)
 * is in ok_S to begin with
 *)

(* lifter definitions *)

locale liftingok_sig =
  fixes l :: "('syn, 'a, 'b :: {Pord_Weak, Okay}) lifting"

locale lifting_validok_weak = liftingok_sig +
  assumes put_ok_S : "\<And> s a b . LUpd l s a b \<in> ok_S"
...
locale lifting_validok_ext = liftingok_sig +
  assumes get_put : "\<And> s a b . b <[ LUpd l s a b"

locale lifting_valid = lifting_validok_weak + lifting_validok_ext

locale lifting_validok_base_ext = liftingok_sig +
...
interpretation triv_l : 
  lifting_valid_weak "(triv_l :: ('x, 'a :: Bogus, 'a md_triv) lifting)" "\<lambda> _ . ok_S"
proof
  fix s :: 'x
  fix a :: 'a
  fix b
...
next
  ...
  fix a :: "'a"
  fix b :: "'a md_triv"
  show "LUpd triv_l s a b \<in> ok_S" by (auto simp add: triv_ok_S)
qed


interpretation triv_l:
  lifting_valid_ok_ext "(triv_l :: ('x, 'a :: {Bogus, Okay}, 'a md_triv) lifting')" "\<lambda> _ . ok_S"
proof
  show "\<And> S . ok_S \<subseteq> ok_S" by auto
next
  fix s a b
...
interpretation triv_l :
  lifting_valid_pres_ext "(triv_l :: ('x, 'a :: {Bogus}, 'a md_triv) lifting')" "\<lambda> _ . ok_S"
proof
next
  fix v :: "'a md_triv"
  fix V 
...
qed
*)
interpretation triv_l : lifting_valid_pairwise_ext "\<lambda> _ . (ok_S :: 'a md_triv set)"
proof
