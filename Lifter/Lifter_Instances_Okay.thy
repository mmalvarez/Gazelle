theory Lifter_Instances_Okay imports Lifter_Okay "../Mergeable/Mergeable_Facts"
begin
...
(*
 * id
 * we need id_l lifting that takes a lifting
 *)
(*
definition id_l ::
  "('x, 'a :: {Pord, Bogus, Okay}, 'a, ('x \<Rightarrow> 'a \<Rightarrow> 'a)) lifting" where
"id_l =
  LMake (\<lambda> s a a' . a) (\<lambda> s a . a) (\<lambda> s . bogus) (\<lambda> f x . f x)" 

interpretation id_l: lifting_valid_weak "id_l"
proof
  show "\<And>s a b. LOut id_l s (LUpd id_l s a b) = a"
    by(auto simp add: id_l_def)
next
  show "\<And>s b. b \<in> ok_S \<Longrightarrow>
           b <[ LUpd id_l s (LOut id_l s b) b"
    by(auto simp add: id_l_def leq_refl)
next
  fix s a b
  show "\<And> s a b . b \<in> ok_S \<Longrightarrow> LUpd id_l s a b \<in> ok_S"
    apply (auto simp add: id_l_def)
qed
*)
...
interpretation triv_l : 
  lifting_valid_weak "(triv_l :: ('x, 'a :: {Bogus, Okay}, 'a md_triv , 'x \<Rightarrow> 'a \<Rightarrow> 'a) lifting)" 
proof
  fix s :: 'x
  fix a :: 'a
  fix b
...
interpretation triv_l :
  lifting_valid_weak_pres "(triv_l :: ('x, 'a :: {Okay, Bogus}, 'a md_triv) lifting')"
proof
next
  fix v :: "'a md_triv"
  fix V 
...
interpretation triv_l : lifting_valid_weak_ok_pres "(triv_l :: ('x, 'a :: {Bogus, Okay}, 'a md_triv) lifting')"
proof
qed
...
sublocale option_l_valid_weak \<subseteq> out : lifting_valid_weak_base "option_l l" "\<bottom>"
proof
...
next
  ...
  show "LUpd (option_l l) s a b \<in> ok_S"
  proof(cases b)
    case None
    then show ?thesis
      using put_S[OF A]
...
next
  ...
  then show " b <[ LUpd (option_l l) s (LOut (option_l l) s b) b"
    using get_put_weak[OF B(2)]
...
sublocale option_l_valid_weak_base \<subseteq> out: lifting_valid_weak_base "option_l l" "\<bottom>"
proof
qed

locale option_l_valid = lifting_valid + option_l_valid_weak

sublocale option_l_valid \<subseteq> param : lifting_valid_base "option_l l" "\<bottom>"
proof
  fix s a b
...
sublocale option_l_valid_weak_base_ok \<subseteq> out : lifting_valid_weak_ok "option_l l"
proof
  fix s a b
...
sublocale option_l_valid_weak_base_ok_pres \<subseteq> out : lifting_valid_weak_base_ok_pres "option_l l" "\<bottom>"
proof
  fix V
  fix s 
  fix v supr :: "'c option"
  ...
  assume Nemp : "v \<in> V"
  assume V_valid : "V \<subseteq> ok_S"

  assume Hsup : "is_sup V supr"
  assume Hsup_in : "supr \<in> ok_S"

  obtain supr' where Supr' : "supr = Some supr'" "supr' \<in> ok_S"
    using V_valid Hsup_in by(auto simp add: option_ok_S)
  ...
  proof(rule is_supI)
    ...
    obtain xo' where Xo' : "xo = Some xo'" "xo' \<in> ok_S" using V_valid Xo
    ...
    hence Conc' : "is_sup (LMap l f s ` {xo', supr'}) (LMap l f s supr')"
      using Xo' Supr' 
      using pres[of xo' "{xo', supr'}" supr' f s]
      by auto

    obtain x' where X' : "x = Some x'"
      using X
      by(cases x; auto simp add: option_l_def split: option.splits)

    show "x <[ LMap (option_l l) f s supr"
  ...
  next
    ...
    obtain V' where SV' : "V = Some ` V'" "V' \<subseteq> ok_S"
      using V_valid
      by(auto simp add: option_ok_S; blast)
  ...
  qed
next
  show "\<And> s . (\<bottom> :: 'c option) \<notin> ok_S"
    by(auto simp add: option_ok_S  option_bot)
qed

locale option_l_valid_base_ok_pres =
  option_l_valid_weak_base_ok_pres + option_l_valid

sublocale option_l_valid_base_ok_pres \<subseteq> out : lifting_valid_base_ok_pres "option_l l" \<bottom>
proof
qed
...
sublocale prio_l_valid_weak \<subseteq> out : lifting_valid_weak "prio_l f0 f1 l"
proof
  fix s a b
...
next
  ...
  show "LUpd (prio_l f0 f1 l) s a b \<in> ok_S"
    using put_S[OF Hb]
    by(auto simp add:prio_l_def prio_ok_S prio_pleq split:md_prio.splits)
next
  fix s 
  fix a :: 'b
  fix b :: "'c md_prio"
  assume Hb : "b \<in> ok_S"
  then obtain pb' b' where B' : "b = mdp pb' b'" "b' \<in> ok_S"
    by(cases b; auto simp add: prio_ok_S)

  show "b <[
           LUpd (prio_l f0 f1 l) s
            (LOut (prio_l f0 f1 l) s b) b"
    using get_put_weak[OF B'(2)] B' f1_nondecrease
    by(auto simp add:prio_l_def prio_ok_S prio_pleq split:md_prio.splits)
qed

locale prio_l_valid = prio_l_valid_weak + lifting_valid

sublocale prio_l_valid \<subseteq> out : lifting_valid "prio_l f0 f1 l" 
proof
  fix s a
  fix b :: "'c md_prio"
(*
...
sublocale prio_l_valid_strong \<subseteq> out : lifting_valid "prio_l f0 f1 l"
proof
  fix s a 
  fix b :: "'c md_prio"
(*  assume H: "b \<in> prio_l_S S s"*)
...
sublocale prio_l_valid_weak_base \<subseteq> out : lifting_valid_weak_base "prio_l f0 f1 l" \<bottom>
proof
  fix s
...
sublocale prio_l_valid_base \<subseteq> out : lifting_valid_base "prio_l f0 f1 l" \<bottom>
proof
  fix s a
  fix b :: "'c md_prio"
  show "b <[ LUpd (prio_l f0 f1 l) s a b"
...
sublocale prio_l_valid_strong_base \<subseteq> out : lifting_valid_base "prio_l f0 f1 l" \<bottom>
proof
qed
...
sublocale prio_l_valid_weak_ok \<subseteq> out : lifting_valid_weak_ok "(prio_l f0 f1 l)" 
proof
  fix s a
  fix b :: "'c md_prio"
  assume H: "b \<in> ok_S"
...
sublocale prio_l_valid_weak_base_ok \<subseteq> out : lifting_valid_weak_base_ok "(prio_l f0 f1 l)" \<bottom>
proof
qed

locale prio_l_valid_ok = prio_l_valid_weak_ok + prio_l_valid

sublocale prio_l_valid_ok \<subseteq> out : lifting_valid_ok "(prio_l f0 f1 l)" 
proof
qed

locale prio_l_valid_base_ok = prio_l_valid_ok + prio_l_valid_weak_base_ok
sublocale  prio_l_valid_base_ok \<subseteq> out : lifting_valid_base_ok "(prio_l f0 f1 l)" \<bottom>
proof
qed

locale prio_l_valid_strong_ok = prio_l_valid_weak_ok + prio_l_valid_strong

sublocale prio_l_valid_strong_ok \<subseteq> out : lifting_valid_ok "prio_l f0 f1 l" 
proof
qed

locale prio_l_valid_strong_base_ok = prio_l_valid_strong_ok + prio_l_valid_strong_base
sublocale prio_l_valid_strong_base_ok \<subseteq> lifting_valid_base_ok "prio_l f0 f1 l" \<bottom>
proof
qed
...
sublocale prio_l_valid_base_ok_pres \<subseteq> out : lifting_valid_base_ok_pres "prio_l f0 f1 l" \<bottom>
proof
(* TODO: this proof can almost definitely be drastically simplified. *)
  ...
  obtain supr' psupr' where Supr' : "supr = mdp psupr' supr'" "supr' \<in> ok_S"
    using V_valid Hsup_in
    by(auto simp add: prio_ok_S split:md_prio.splits)

  have Vsubs : "V \<subseteq> ({ xp . (case xp of mdp p x \<Rightarrow> x \<in> ok_S)})"
    using V_valid
    by(auto simp add: prio_ok_S)

  then obtain V' where SV' : "(\<lambda> x . case x of (mdp p m) \<Rightarrow> m) ` V = V'"
    by(blast)

  then have SV'_subs : "V' \<subseteq> ok_S"
    using V_valid
    by(auto simp add: prio_ok_S split: md_prio.splits)
  ...
  proof(rule is_supI)
    ...
    have "xo \<in> ok_S"
      using V_valid Xo by auto

    obtain xo' pxo' where Xo' : "xo = mdp pxo' xo'" "xo' \<in> ok_S" using Xo Vsubs
      by(cases xo; auto simp add: prio_ok_S split: md_prio.splits)

(*
    have "x \<in> ok_S" 
      using put_S Xo pres
      apply(auto simp add: prio_ok_S prio_l_def split: md_prio.splits)
...
*)
    ...
    proof(cases "pxo' = psupr'")
      ...
      hence Conc' : "is_sup (LMap l f s ` {xo', supr'}) (LMap l f s supr')"
        using Xo' Supr' pres[of xo' "{xo', supr'}" supr' f s] 
        by auto

      then show ?thesis
        using is_supD1[OF Conc']  Xo Xo' Supr'
    ...
    next
      ...
      obtain px' x' where X' : "x = mdp px' x'"
        by(cases x; auto)
      ...
      then show ?thesis using X' Xo Xo' Supr'  Result
  ...
  next
    ...
    proof(cases "has_sup Vmaxv")
      ...
      hence False using bot_bad Supr' bot_alt_is_bot
        by auto
  ...
  qed
next
  show "(\<bottom> :: 'c md_prio) \<notin> ok_S" using bot_bad bot_alt_is_bot
    by(auto simp add: prio_bot prio_ok_S)
qed
...
sublocale fst_l_valid_weak \<subseteq> out : lifting_valid_weak "fst_l l"
proof
