...
definition merge_l ::
  "('x, 'a1, ('b1 :: Mergeable) * ('b2 :: Mergeable)) lifting \<Rightarrow>
   ('x, 'a2, 'b1 * 'b2) lifting \<Rightarrow>
...
locale merge_l_valid_weak' = 
  fixes l1 :: "('x, 'a1, ('b1 :: Mergeable * 'b2 :: Mergeable)) lifting" 
  fixes l2 :: "('x, 'a2, 'b1 * 'b2) lifting"
...
proof
  fix v supr :: "'c * 'd"
  fix V
  fix f :: "'a \<Rightarrow> 'b * 'e \<Rightarrow> 'b * 'e"
  fix s :: 'a
  assume Vin : "v \<in> V"
...
*)
  ...
  (* new plan: come up with a function that gets the second component
     given a first component (e.g.) *)

(* alternatively we should see what happens if we force the output type
   to be a product. *)
...
*)
  ...
  proof(rule is_supI)
    ...
    obtain x1a x2a where X12a : "x = (x1a, x2a)"
      by(cases x; auto)
