theory Auto_Lifter
    imports Main "../Mergeable/Mergeable" Lifter_Instances "HOL-Library.Adhoc_Overloading"
begin
 ...
 * (which, again, doesn't support backtracking) to implement "or"-like behavior, but we can get
 * equivalent behavior (for a closed world of names) if we negate, so that
 * "does the first or second component contain nA" becomes
 * "does neither the first nor the second component contain nA".
 * Isabelle's typeclass algorithm handles the latter naturally.
 *)
 ...
 * and leave it to the left-hand-side automation to actually do the merging.
 *)
