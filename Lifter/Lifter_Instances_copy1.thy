...
proof
  ...
  then show "LUpd (merge_l l1 l2) s a b \<in> S1 s \<inter> S2 s" using A in2.put_S
    apply(simp add: merge_l_def compat)
...
*)
(* this is maybe promising, but because we have lost the 1-1 correspondence of
 * values it may not actually suffice.
  ...
  proof(rule is_supI)
    ...
    have True
      using compat_pres1'[OF Vin Vsub1 Vsub2]

    show "x <[ (case (x1, x2) of (x1, x2) \<Rightarrow> (LUpd l1 s x1 (LUpd l2 s x2 supr)))"
      using Xo Xr X12
      using is_supD1[OF
in1.pres[OF Xo(1) Vsub1 Supr Supr_in1,
of "\<lambda> s x . 
      (if x = (LOut l1 s supr) then fst (f s (LOut l1 s supr, LOut l2 s supr))
       else fst (f s (LOut l1 s xo, LOut l2 s xo)))"]
imageI[OF Xo(1)]]
      using is_supD1[OF
in2.pres[OF Xo(1) Vsub2 Supr Supr_in2,
of "\<lambda> s x . 
      (if x = (LOut l2 s supr) then snd (f s (LOut l1 s supr, LOut l2 s supr))
       else snd (f s (LOut l1 s xo, LOut l2 s xo)))"]
imageI[OF Xo(1)]]
      apply(auto split: prod.splits if_split_asm)
*)
...
*)
  ...
  proof(rule is_supI)
    ...
    then have "x \<in> (\<lambda>x. case f s (LOut l1 s x, LOut l2 s x) of (a1, a2) \<Rightarrow> [^ LUpd l1 s a1 x, LUpd l2 s a2 x ^]) ` V"
      by(auto simp add: merge_l_def)

    then obtain xo where Xo : "xo \<in> V" "(\<lambda>x. case f s (LOut l1 s x, LOut l2 s x) of (a1, a2) \<Rightarrow> [^ LUpd l1 s a1 x, LUpd l2 s a2 x ^]) xo  = x"
      by(auto)

    obtain xr1 xr2 where Xr : "(xr1, xr2) = f s (LOut l1 s xo, LOut l2 s xo)"
      using Xo by( auto split: prod.splits)

    have Xo' : "[^ LUpd l1 s xr1 xo, LUpd l2 s xr2 xo ^] = x"
      using Xo Xr
      by(auto split: prod.splits)

    show "x <[ LMap (merge_l l1 l2) f s supr" using X12 unfolding sym[OF Xo']
      apply(simp add: merge_l_def)
...
(* YOU ARE HERE
 * maybe we can prove this from this point?
*)
(*
...
a <[ b \<Longrightarrow> Upd l1 a <[ Upd l1 b
*)

(*
(*is_supD1[OF in2.pres[OF Xo(1) Vsub2 Supr Supr_in2, of "\<lambda> s x . snd( f s (LOut l1 s xo, x))"]
imageI[OF Xo(1)]]*)
is_supD1[OF in1.pres[OF Xo(1) Vsub1 Supr Supr_in1] imageI[OF Xo(1)],
of "\<lambda> s x . fst (f s (x, LOut l2 s supr))"]
is_supD1[OF in1.pres[OF Xo(1) Vsub1 Supr Supr_in1] imageI[OF Xo(1)],
of "\<lambda> s x . fst (f s (x, LOut l2 s xo))"]
is_supD1[OF in2.pres[OF Xo(1) Vsub2 Supr Supr_in2] imageI[OF Xo(1)],
of "\<lambda> s x . snd( f s (LOut l1 s supr, x))"]
is_supD1[OF in2.pres[OF Xo(1) Vsub2 Supr Supr_in2] imageI[OF Xo(1)],
of "\<lambda> s x . snd( f s (LOut l1 s xo, x))"]
*)

      apply(auto split: prod.splits)
      apply(cases "(f s (LOut l1 s supr, LOut l2 s xo))")
      apply(cases "(f s (LOut l1 s xo, LOut l2 s supr))")
      apply(auto)

(* YOU ARE HERE
maybe we can say something or add a property about
Up1 (Up2 _) <[ Up1 (Up2 _)
meaning that just Up1 and just Up2 also \<le> ?
*)
