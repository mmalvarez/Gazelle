theory Hoare_Indexed_Sound2 imports Hoare_Indexed Hoare
begin
...
"HT' gs P c Q =
  ((\<forall> n . |#gs#| {#- P, (n) -#} c {#- Q, n -#}))"

lemma HT'I :
  assumes H : "(\<And> n. |#gs#| {#- P, (n) -#} c {#- Q, n -#})"
...
proof
  ...
  proof
    ...
    proof
      ...
      assume "P (payload m)"
      assume "cont m = Inl (c @ c') "
      show "safe_for gs m n"
        using HTE[OF H]

(* idea: if c' isn't safe under Q, there must exist some n' > n where we witness unsafeness *)
  ...
  next
    ...
    proof(rule HTiI)
      fix c'
      assume X:  "|#gs#| {#Q, Suc n#} c'"
      hence X' : "|#gs#| {#Q, n#} c'" sorry

      have Conc' :  "|#gs#| {#P, n#} (c @ c')"
        using HTiE[OF Suc.IH[OF Suc.prems] X']
        by auto



      show "|#gs#| {#P, Suc n#} (c @ c')" using HTE[OF Suc.prems]
(* if c' unsafe under Q, then we can find a smallest n where it is not safe for n steps
 * 
 *)
  qed
