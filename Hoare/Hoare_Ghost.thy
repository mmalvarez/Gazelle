...
lemma HConseq :
  ...
  assumes H' : "\<And> st g . P st g \<Longrightarrow> P' st g"
  assumes H'' : "\<And> st g . Q' st g \<Longrightarrow> Q st g"
  shows "|gs| {- R -} {=P=} c {=Q=}"
proof(rule HTI)
  fix c' g g'
  ...
  then show "|gs| {P} g (c @ c')"
    unfolding guarded_def using H' by blast
qed

lemma HCat :
  assumes H : "|gs| {-R1-} {= P1 =} c1 {= P2 =}"
  assumes H' : "|gs| {-R2-} {= P2 =} c2 {= P3 =}"
  shows "|gs| {- (R1 OO R2) -} {= P1 =} c1 @ c2 {= P3 =}"
proof(rule HTI)
  fix c'
  fix g g''
  ...
  assume HP3 : "|gs| {P3} g'' c'"

  have P2_cont : "|gs| {P2} g' (c2 @ c')"
    using HTE[OF H' G'2 HP3]
    by auto

  show "|gs| {P1} g ((c1 @ c2) @ c')"
    using HTE[OF H G'1 P2_cont]
