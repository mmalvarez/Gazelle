...
lemma sups_presD :
  ...
  shows "(has_sup ((\<lambda> f . f syn x) ` Fs'))" using assms
...
proof(rule sups_presI)
  ...
  have Hnemp_Fs' : "f \<in> Fs'"
    using Hnemp_Fs_sub Hfs_sub Rest
    by auto

  have Sub2: "Fs_sub \<subseteq> Fs"
    using Hsub Rest Hfs_sub
    by auto

  show "has_sup
        ((\<lambda>f. f syn x) ` Fs_sub)"
    using sups_presD[OF H Subs Sub2 Hnemp_Fs_sub] Hfs_sub Hnemp_Fs_sub Hsub 
...
next
  ...
  proof(cases t1)
    ...
    obtain sup' where Sup' : "is_sup ((\<lambda>f. f syn sem) ` {h1}) sup'" 
      using Cons.prems sups_presD[OF SP, of "sem" syn "{h1}" h1]
      by(auto simp add: sup_singleton has_sup_def)

    then show ?thesis using Nil' Cons.prems
  ...
  next
    ...
    have Sing_sem : "sem \<in> S syn" using Cons.prems by auto

    obtain sup' where Sup' :
      "is_sup ((\<lambda>f. f syn sem) ` set (h1 # t1)) sup'"
      using sups_presD[OF Cons.prems(1) Sing_sem, of "set (h1#t1)" h1]
      by(auto simp add: has_sup_def)

    have Union : "is_sup ({h1 syn sem} \<union> (\<lambda>f. f syn sem) ` set t1) sup'" using Sup' 
  ...
  "(insert (h2 syn sem) (insert (h1 syn sem) ((\<lambda>x. x syn sem) ` set t2))) = 
   (insert (h1 syn sem) (insert (h2 syn sem) ((\<lambda>x. x syn sem) ` set t2)))"
    ...
    show ?thesis using Conc1 by auto
  qed
qed
...
next
  ...
  proof(cases ft1)
    ...
    then show ?thesis using Nil' Sup'1_eq
      by(auto simp add: scross_singleton1)
  next
    ...
    obtain sup' where Sup' :
      "is_sup (scross ((\<lambda>f. f syn) ` set (fh1 # ft1)) Xs) sup'" 
...
proof-
  ...
  obtain supr where
    Supr1 : "is_sup ((\<lambda>f. f syn y) ` {f}) supr"
    using sups_presD[OF H ] Hf Hx_in Hy_in
    by auto
...
proof(rule sups_presI)
  ...
  then have Nemp'' : "l1 @ l2 \<noteq> []"
    by auto

  have Sup : "is_sup ((\<lambda>f. f syn sup1) ` set (l1 @ l2)) (pcomps (l1 @ l2) syn sup1)"
    using sups_pres_pcomps_sup'[OF Pres' Nemp'' H'_in]
    by simp
  ...
  have Sup1 : "is_sup ((\<lambda>f. f syn sup1) ` set l1) (pcomps l1 syn sup1)"
    using sups_pres_pcomps_sup'[OF Pres1 F1' H'_in]
    by auto

  have Sup2 : "is_sup ((\<lambda>f. f syn sup1) ` set l2) (pcomps l2 syn sup1)"
    using sups_pres_pcomps_sup'[OF Pres2 F2' H'_in]
    by auto

  have Unfold : "(\<lambda>f. f syn sup1) ` set l1 \<union> (\<lambda>f. f syn sup1) ` set l2 = (\<lambda> f . f syn sup1) ` set( l1 @ l2)"
    by(auto)

  have Sup12 : "is_sup ((\<lambda>f. f syn sup1) ` {pcomps l1, pcomps l2}) (pcomps (l1 @ l2) syn sup1)"
    using sup_union2[OF Sup1 Sup2, of "pcomps (l1 @ l2) syn sup1" ] Sup
    unfolding Unfold
    by(auto)
  ...
  then show "has_sup ((\<lambda>f. f syn sup1) ` Fs_sub)"
  proof cases
    case Emp
