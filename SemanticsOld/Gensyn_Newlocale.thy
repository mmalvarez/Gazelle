...
(*
  fixes base_sem' :: 
  "'barb itself \<Rightarrow>
   'rarb itself \<Rightarrow>
   'garb itself \<Rightarrow>
  'g \<Rightarrow> 
  'b \<Rightarrow>
  'mstate \<Rightarrow> 
  'mstate \<Rightarrow> 
  childpath \<Rightarrow>
  ('barb, 'rarb, 'garb) gensyn \<Rightarrow>  
  ('res) gs_result \<Rightarrow>
  bool"
...
inductive gensyn_sem ::
  "'b itself \<Rightarrow> 'r itself \<Rightarrow> 'g itself \<Rightarrow>
   ('b, 'r, 'g) gensyn \<Rightarrow>
   childpath \<Rightarrow>
   'mstate \<Rightarrow>
   'mstate \<Rightarrow>
...
interpretation Gensyn_Semantics_Nosem : GSTest
  nosem_base_sem nosem_rec_sem
  done

definition thingy :: "(nat, unit, bool) gensyn" where
"thingy = GBase True (0 :: nat)"

lemma test :
  "Gensyn_Semantics_Nosem.gensyn_sem (_ itself) _ _ _ _ thingy [] () ()"
