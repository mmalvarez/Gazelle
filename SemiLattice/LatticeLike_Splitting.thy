theory LatticeLike_Splitting imports Main
...
"is_bub_sup a b s =
  (lleq a s \<and>
    ((\<forall> S ord s' . is_splitting S \<longrightarrow>
            ord \<in> S \<longrightarrow>
            LatticeLike.is_sup ord {a, b} s' \<longrightarrow>
            ord s' s)))"
...
*)
  assumes bsup_is_bsup_sup:
    "\<And> a b . is_bsup_sup a b (bsup a b)"
(*
...
lemma bsup_sup_sup :
  ...
  apply(simp add: is_bsup'_def is_bub'_def has_sup_def is_sup_def is_bsup_def is_bub_def is_ub_def is_least_def
        ...
        is_bub_new_def is_bsup_new_def is_bsup_sup_def is_bub_sup_def) apply(auto)
   apply(drule_tac x = "{lleq}" in spec) apply(auto)
    apply(simp add:is_splitting_def) apply(simp add:LatticeLike_Weak_Spec_axioms)
