theory LatticeLike_sup_weak imports Main
...
*)
  assumes bsup_is_weak:
    "\<And> a b . is_bsup_weak a b (bsup a b)"
(*
...
lemma bsup_sup_weak :
  "\<And> a b x .is_bsup_weak a b x \<Longrightarrow> has_sup {a, b} \<Longrightarrow> 
             is_sup {a, b} x"
  apply(simp add:has_sup_def is_sup_def  is_ub_def is_least_def
...
interpretation Test0 : Mergeable_Spec test0_lleq test0_bsup
  ...
  apply(simp add:Mergeable'.is_bsup_weak_def  LatticeLike.is_least_def LatticeLike.is_sup_def LatticeLike.is_ub_def
  ...
  apply(auto)
   ...
   apply(case_tac a; clarsimp)
    ...
    apply(case_tac b; clarsimp)
     ...
     apply(case_tac "test0_lleq (Some a) (Some aa)")      apply(auto)
