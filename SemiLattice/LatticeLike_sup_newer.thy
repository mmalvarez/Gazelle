theory LatticeLike_sup_newer imports Main
...
definition abstracts :: "(('a \<Rightarrow> 'a set) \<Rightarrow> bool)" where
"abstracts f =
  ((\<forall> x . x \<in> f (x)) \<and>
   (\<forall> x x' . f x = f x' \<or> (f x \<inter> f x' = {})))"

definition ord_abstract :: "('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow>
                             ('a \<Rightarrow> 'a set) \<Rightarrow>
                             ('a \<Rightarrow> 'a \<Rightarrow> bool)" where
"ord_abstract ord abst a b =
  (\<forall> a' . a' \<in> abst a \<longrightarrow> 
      (\<exists> b' . b' \<in> abst b \<and> ord a' b'))"
...
"is_bub a b s =
  (lleq a s \<and>
    ((\<forall> lleq' s' . ord_leq lleq lleq' \<longrightarrow>
                preserves_maxes lleq lleq' \<longrightarrow>
                LatticeLike_Weak_Spec lleq' \<longrightarrow>
                LatticeLike.is_sup lleq' {a, b} s' \<longrightarrow>
...
- greater than a
- monotonic in b
- something about orderings (another idea: abstract into sets)
*)
...
"is_bub a b s =
  (lleq a s \<and>
    ((\<forall> abst lleq' s' .
                abstracts abst \<longrightarrow>
                lleq' = ord_abstract lleq abst \<longrightarrow>
                LatticeLike.is_sup lleq' {a, b} s' \<longrightarrow>
                lleq' s' s)))"
...
lemma bsup_sup :
  ...
  apply(simp add:has_sup_def is_sup_def is_bsup_def is_bub_def is_ub_def is_least_def
   ...
   apply(drule_tac x = "(\<lambda> a . {a})" in spec) apply(auto)
  apply(simp add:abstracts_def)
  
  apply(rotate_tac -1)
  apply(drule_tac x = s in spec) apply(auto)
    apply(simp add:ord_abstract_def) 
     apply(simp add:LatticeLike.is_ub_def) apply(auto)
      apply(simp only:ord_abstract_def) apply(clarify)

apply(drule_tac x = s in spec) apply(safe)
  apply(simp)

    apply(simp add:LatticeLike.is_ub_def abstracts_def ord_abstract_def) 
  ...
  apply(drule_tac x = a' in spec) apply(auto)
    apply(simp add:LatticeLike.is_ub_def ord_abstract_def)
  apply(auto)

  apply(drule_tac x = abst in spec) apply(auto)
  apply(rotate_tac -1)
  apply(drule_tac x = s in spec) apply(auto)
    apply(rule_tac x = a in exI) apply(simp add:abstracts_def)
    apply(rule_tac x = s in exI) apply(simp add:abstracts_def)

    apply(rule_tac x = b in exI) apply(simp add:abstracts_def)
    apply(rule_tac x = s in exI) apply(simp add:abstracts_def)
...
apply(drule_tac x = a' in spec) apply(auto)
  ...
  apply(auto) apply(simp add:abstracts_def)
...
(* idea: some x2 is a max *)
  ...
  apply(auto)
  apply(drule_tac LatticeLike_Weak_Spec.leq_refl) apply(simp)
  apply(simp add:test0_lleq_def)
...
(* old proof attempt *)
  ...
  apply(simp add:test_bsup_def)
    apply(case_tac a; clarsimp)
  apply(simp add:test_lleq_def) apply(clarsimp)
     apply(case_tac ab; auto)
      apply(drule_tac x = None in spec)
      apply(drule_tac x = bb in spec)
     apply(auto)
