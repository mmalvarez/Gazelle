theory LatticeLike_Closure imports Main HOL.Transitive_Closure
...
definition add_eqs :: "('a * 'a) set \<Rightarrow> ('a * 'a) set \<Rightarrow> ('a * 'a) set" where
"add_eqs ordS eqS =
  rtrancl (ordS \<union> eqS \<union> {(a, b) . (b, a) \<in> eqS})"
...
"is_bub a b s =
  (lleq a s \<and>
    ((\<forall> lleq'S eqs s' . lleq'S = add_eqs {(a, b) . lleq a b} eqs \<longrightarrow>
                LatticeLike.is_sup (\<lambda> a b . (a, b) \<in> lleq'S) {a, b} s' \<longrightarrow>
                (s', s) \<in> lleq'S)))"
...
lemma bsup_sup :
  ...
  apply(drule_tac x = s in spec)
   apply(simp add:add_eqs_def) apply(auto)
     apply(simp add: LatticeLike.is_ub_def) apply(auto)
    apply(simp add: LatticeLike.is_ub_def) apply(auto)
    apply(simp add:rtrancl_def) defer (* rtranclp lleq = lleq *)

    apply(simp add:rtrancl_def) defer (* rtranclp lleq = lleq *)

    apply(drule_tac x = s in spec) apply(auto)
     apply(simp add:add_eqs_def rtrancl_def)

   apply(drule_tac x = "eqs" in spec) apply(rotate_tac -1)
  apply(drule_tac x = s in spec)
     apply(auto)
  ...
  apply(drule_tac x = a'a in spec) apply(auto)
     ...
     apply(rotate_tac -2) apply(drule_tac x = s' in spec) apply(auto)
     apply(simp add:LatticeLike.is_ub_def)
     apply(auto)
  apply(drule_tac x = s' in spec) apply(auto)

    apply(simp add:add_eqs_def)

  apply(rule_tac a = b and b = "bsup a b" in  rtrancl_induct)
     apply(auto)
      apply(simp add:leq_refl)
  apply(rule_tac leq_trans; auto)
  apply(simp add: add_eqs_def)
  apply(rotate_tac -2) apply(drule_tac CollectD)
