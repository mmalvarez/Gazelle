theory LatticeLike_Neweqs_weaker imports Main HOL.Transitive_Closure
...
"ord_extends o1 o2 =
  (ord_leq o1 o2 \<and>
  (\<forall> x1 x2 . o2 x1 x2 \<longrightarrow>
            \<not>(o1 x1 x2) \<longrightarrow>
             (\<exists> c c' . o1 x1 c \<and> o1 c' x2 \<and> o2 c c' \<and> o2 c' c \<and> c \<noteq> c')))"
...
lemma bsup_sup :
  ...
  apply(drule_tac x = s in spec)
   apply(auto)
     apply(simp add: LatticeLike.is_ub_def is_ub_def)
  ...
  apply(rotate_tac -1)
   ...
   apply(case_tac c'; auto)
     apply(frule_tac a ="Some aa" and b = None and c = "Some ab" in LatticeLike_Weak_Spec.leq_trans)
       apply(clarsimp)
      apply(drule_tac a = None and b = "Some ab" in ord_leq'; clarsimp)

  apply(drule_tac x = "Some ab" in spec) apply(clarsimp)

  apply(simp add:LatticeLike_Weak_Spec.leq_refl)
   apply(frule_tac a ="Some a" and b = "Some ab" and c = "Some aa" in LatticeLike_Weak_Spec.leq_trans)
  apply(simp) apply(simp)
