theory LatticeLike_scrunch imports Main
...
definition is_mono :: "('a \<Rightarrow> 'a) \<Rightarrow> bool"
  where
"is_mono f =
  (\<forall> a a' . lleq a a' \<longrightarrow> lleq (f a) (f a'))"

(*
definition is_bub :: "'a \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> bool" where
"is_bub a b s =
  (lleq a s \<and>
    (\<exists> proj :: ('a \<Rightarrow> 'a) . is_mono proj \<and> 
     (\<exists> s' .
       is_sup {proj a, proj b} s' \<and>
       lleq s' (proj s))))"
*)
...
"is_bub a b s =
  (lleq a s \<and>
    (\<forall> proj :: ('a \<Rightarrow> 'a) . \<forall> s' . is_mono proj \<longrightarrow>
       is_sup {proj a, proj b} s' \<longrightarrow>
       lleq s' (proj s)))"
...
lemma bsup_sup :
  ...
  apply(rotate_tac -1)
    ...
    apply(drule_tac x = "proj a'" in spec, auto)
   apply(simp add:is_mono_def) apply(simp add:is_mono_def)
  done
*)
...
lemma bsup_sup :
  ...
  apply(simp add:has_sup_def is_sup_def is_bsup_def is_bub_def is_ub_def is_least_def
        LatticeLike.is_sup_def LatticeLike.is_least_def) apply(auto)
   apply(rotate_tac -1)
   apply(frule_tac x = "s" in spec) apply(auto)
...
interpretation Test0 : Mergeable_Spec test0_lleq test0_bsup
  ...
  apply(auto)
     ...
     apply(simp add:test0_lleq_def) apply(simp split:option.splits)
      apply(simp add:Mergeable'.is_mono_def split:option.splits)
  apply(auto)
    apply(drule_tac x = id in spec) apply(clarsimp)
  ...
  apply(case_tac "proj b"; auto)
   apply(simp add:test0_lleq_def split:option.splits)
apply(simp add:test0_lleq_def split:option.splits)
   apply(auto)
  apply(drule_tac x = "Some x2" in spec) apply(auto)
   apply(case_tac "b") apply(clarsimp) apply(auto)
  apply(drule_tac x = 
    apply(rotate_tac 1) apply(drule_tac x = None in spec) apply(clarsimp)
    apply(simp add:test0_lleq_def)

   apply(clarsimp) apply(simp add:test0_lleq_def)
  apply(case_tac b; clarsimp)
    apply(simp split:option.splits)
  apply(drule_tac x = None in spec) apply(auto)
