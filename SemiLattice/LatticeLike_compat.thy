theory LatticeLike_compat imports Main
...
locale LatticeLike =
  ...
  fixes nlleq :: "'a \<Rightarrow> 'a \<Rightarrow> bool"
...
locale LatticeLike_Weak_Spec =
  ...
  assumes
    leq_nleq : "\<And> a b . nlleq a b \<Longrightarrow> lleq a b \<Longrightarrow> False"
...
"is_bub a b s =
  (lleq a s \<and>
    ((\<forall> lleq' . ord_leq lleq lleq' \<longrightarrow>
                LatticeLike_Weak_Spec lleq' nlleq \<longrightarrow>
...
lemma bsup_sup :
  ...
  apply(simp add:has_sup_def is_sup_def is_bsup_def is_bub_def is_ub_def is_least_def
        LatticeLike.is_sup_def LatticeLike.is_least_def is_bsup'_def is_bub'_def) apply(auto)
...
lemma bsup_sup :
  "\<And> a b . has_sup {a, b} \<Longrightarrow> is_sup {a, b} (bsup a b)"
    apply(cut_tac a = a and b = b in bsup_is_bsup)
  apply(simp add:has_sup_def is_sup_def is_bsup_def is_bub_def is_ub_def is_least_def
        LatticeLike.is_sup_def LatticeLike.is_least_def)
  apply(auto)   apply(drule_tac x = s in spec) apply(auto)
    apply(simp add:ord_leq_def)
  ...
  apply(simp add:leq_refl)
   ...
   apply(drule_tac a = aa and b = ba in leq_trans) apply(simp) apply(simp)
  apply(drule_tac leq_nleq) apply(simp) apply(simp)
...
(*
definition test0_nlleq :: "nat option \<Rightarrow> nat option \<Rightarrow> bool" where
"test0_nlleq a b =
  (case a of None \<Rightarrow> False
   | Some a' \<Rightarrow> (case b of 
                 None \<Rightarrow> True
                 | Some b' \<Rightarrow> True))"
*)
definition test0_nlleq :: "nat option \<Rightarrow> nat option \<Rightarrow> bool" where
"test0_nlleq a b =
  (case a of None \<Rightarrow> False
   | Some a' \<Rightarrow> (case b of 
                 None \<Rightarrow> True
                 | Some b' \<Rightarrow> (b' \<noteq> a')))"

(*
definition test0_nlleq':: ""nat option \<Rightarrow> nat option \<Rightarrow> bool""
*)
...
interpretation Test0 : Mergeable_Spec test0_lleq test0_nlleq test0_bsup
  apply(unfold_locales)
     apply(simp add:test0_lleq_def) apply(simp split:option.splits)
     apply(simp add:test0_lleq_def) apply(simp split:option.splits)
  apply(simp add:test0_nlleq_def test0_lleq_def)  apply(simp split:option.splits)
    apply(simp add:test0_lleq_def) apply(simp split:option.splits)
  apply(clarsimp)
  apply(simp add:Mergeable'.is_bsup_def Mergeable'.is_bsup'_def Mergeable'.is_bub'_def Mergeable'.is_bub_def LatticeLike.is_least_def LatticeLike.is_sup_def LatticeLike.is_ub_def)
...
(* shouldn't need to reprove this. *)
  ...
  apply(drule_tac x = "Some x2" in spec)
  apply(auto)
   apply(drule_tac a = "Some x2" and b = "Some x2" in ord_leq')
    apply(simp add:test0_lleq_def) apply(auto)
  apply(case_tac b; clarsimp)
   apply(drule_tac a = "None" and b = "Some x2" in ord_leq')
    apply(simp add:test0_lleq_def) apply(auto)

  apply(simp add:LatticeLike_Weak_Spec_def) 
  apply(safe)
  apply(case_tac "a = x2") apply(clarsimp)
  apply(case_tac s') apply(clarsimp)
   apply(rotate_tac -2)
   apply(drule_tac x = "Some x2" in spec) apply(rotate_tac -1)
   apply(drule_tac x = "None" in spec)
   apply(simp add:test0_nlleq_def)

  apply(clarsimp)
  apply(case_tac "aa = x2") apply(clarsimp)
  apply(rotate_tac -3)
  apply(drule_tac x = "Some x2" in spec)
  apply(rotate_tac -1)
...
(*
   ...
   apply(case_tac b; clarsimp)
     apply(drule_tac a = None and b = "Some aa" in ord_leq') apply(simp) apply(simp)
    apply(simp add:ord_leq_def)
    apply(drule_tac a = "Some a" and b = "Some aa" in ord_leq')
    apply(drule_tac x = "Some aa" in spec) apply(clarsimp)
  apply(auto)
  ...
  apply(drule_tac x = "Some aa" in spec) apply(auto)
   apply(simp add:LatticeLike_Weak_Spec_def) 
  apply(simp add:ord_leq_def)
  apply(drule_tac x = "Some a" in spec)
  apply(auto)
    apply(drule_tac a = "Some aa" and b = "Some a" in ord_leq') apply(simp)
...
definition test_nlleq :: "(nat option * nat) \<Rightarrow> (nat option * nat) \<Rightarrow> bool" where
"test_nlleq l r =
  (case (l, r) of
         ((None, l2), (None, r2)) \<Rightarrow> False
       | ((None, l2), (Some r1, r2)) \<Rightarrow> False
       | ((Some _, l2), (None, r2)) \<Rightarrow> l2 \<noteq> r2
       | ((Some l1, l2 ), (Some r1, r2)) \<Rightarrow> l1 \<noteq> r1 \<and> l2 \<noteq> r2)"

definition test_bsup :: "(nat option * nat) \<Rightarrow> (nat option * nat) \<Rightarrow> (nat option * nat)" where
"test_bsup l r = (case (l, r) of
...
interpretation Test : Mergeable_Spec test_lleq test_nlleq test_bsup

  apply(unfold_locales)
  
      apply(simp add:test_lleq_def split:prod.splits option.splits)
     apply(simp add:test_lleq_def split:prod.splits option.splits)
    apply(simp add:test_lleq_def test_nlleq_def split:prod.splits option.splits)
   apply(simp add:test_lleq_def split:prod.splits option.splits)

  (* bsup *)
  apply(simp add: Mergeable'.is_bsup'_def)
  apply(simp add: LatticeLike.is_least_def)
     apply(simp add:LatticeLike.is_sup_def Mergeable'.is_bub'_def LatticeLike.is_least_def
                    LatticeLike.is_ub_def)
  apply(auto)
  apply(simp add:test_bsup_def test_lleq_def)
