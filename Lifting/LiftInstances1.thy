theory LiftInstances1 imports LiftUtils
begin
...
definition lift_comp ::
  "('x, 'z2, 'z3) lifting \<Rightarrow> ('x, 'z1, 'z2) lifting \<Rightarrow>
   ('x, 'z1, 'z3) lifting" (infix "o*" 50) where
"l1 o* l2 =
  \<lparr> LIn1 = (\<lambda> s a . LIn1 l1 s (LIn1 l2 s a))
  , LIn2 = (\<lambda> s a b . LIn2 l1 s (LIn2 l2 s a (LOut1 l1 s b)) b)
  , LOut1 = (\<lambda> s b . LOut1 l2 s (LOut1 l1 s b)) \<rparr>"

lemma lift_comp_valid :
  fixes l1 :: "('x, 'z2, 'z3) lifting"
  fixes l2 :: "('x, 'z1, 'z2) lifting"
  assumes H1 : "lifting_valid l1"
  assumes H2 : "lifting_valid l2"
  shows "lifting_valid (l1 o* l2)"
proof(rule lifting_valid_intro)
  fix s :: 'x
  fix a :: 'z1
  show "LOut1 (l1 o* l2) s (LIn1 (l1 o* l2) s a) = a"
    using lifting_valid_unfold1[OF H1] lifting_valid_unfold1[OF H2]
    by(simp add:lift_comp_def)
next
  fix s :: 'x
  fix a :: 'z1
  fix b :: 'z3
  show "LOut1 (l1 o* l2) s (LIn2 (l1 o* l2) s a b) = a"
    using lifting_valid_unfold2[OF H1]
      lifting_valid_unfold2[OF H2]
    by(simp add:lift_comp_def)
qed
...
definition triv_l ::
  "('x, 'b, 'b md_triv) lifting" where
"triv_l =
  \<lparr> LIn1 = (\<lambda> s a . mdt a)
  , LIn2 = (\<lambda> s a b . (mdt a))
  , LOut1 = (\<lambda> s b . (case b of (mdt b') \<Rightarrow> b'))\<rparr>"
...
(* NB this is going to return undefined
   more often than the parameterized version from LiftInstances?
   we could fix this by changing LOut1 to return an option *)
definition option_l ::
  "('x, 'b, 'b option) lifting" where
"option_l =
  \<lparr> LIn1 = (\<lambda> s a . Some (a))
  , LIn2 = (\<lambda> s a b . Some a)
  , LOut1 = (\<lambda> s b . (case b of Some b' \<Rightarrow> b'))\<rparr>"
...
definition fst_l ::
  "('x, 'a, 'a * ('b2 :: Pordb)) lifting" where
"fst_l =
  \<lparr> LIn1 = (\<lambda> s a . (a, \<bottom>))
  , LIn2 = (\<lambda> s a b . (a, \<bottom>))
  , LOut1 = (\<lambda> s x . (fst x))\<rparr>"
...
*)
definition snd_l ::
  "('x, 'a, ('b1 :: Pordb) * 'a) lifting" where
"snd_l  =
  \<lparr> LIn1 = (\<lambda> s a . (\<bottom>, a))
  , LIn2 = (\<lambda> s a b . (\<bottom>, a))
  , LOut1 = (\<lambda> s x . snd x) \<rparr>"
