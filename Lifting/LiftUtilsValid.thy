...
"lifting_valid l =
  ((\<forall> s a b . LOut l s (LIn l s a b) = a) \<and>
   (\<forall> s a . LNew l s a = LIn l s a (LBase l)))"

lemma lifting_valid_intro :
  assumes H1 : "\<And> s a b . LOut l s (LIn l s a b) = a"
  assumes H2 : "\<And> s a . LNew l s a = LIn l s a (LBase l)"
...
lemma lifting_valid_unfold3 :
  assumes H : "lifting_valid l"
  shows "LNew l s a = LIn l s a (LBase l)"
  using H by (auto simp add:lifting_valid_def)

(* This does not hold for prio_inc
   Do we need it? *)
definition liftingv_valid :: "('x, 'a, 'b, 'z) liftingv_scheme \<Rightarrow> bool" where
"liftingv_valid l =
  ((lifting_valid l) \<and>
   (\<forall> s a b . LIn l s a b \<in> LOutS l s) \<and>
   (\<forall> s b . 
    b \<in> LOutS l s \<longrightarrow>
    LIn l s (LOut l s b) b = b))"

lemma liftingv_valid_intro :
  assumes H1 : "lifting_valid l"
  assumes H2 : "\<And> s a b . LIn l s a b \<in> LOutS l s"
  assumes H3 : "\<And> s b . b \<in> LOutS l s \<longrightarrow> LIn l s (LOut l s b) b = b"
...
lemma liftingvm_valid_intro :
  assumes H1 : "liftingv_valid l"
  assumes H2 :
    "\<And> s a a' b b' .
      b' \<in> LMgS l s b \<longrightarrow>
      a = LOut l s b \<longrightarrow>
  ...
  shows "liftingvm_valid l" using H1 H2
    by(auto simp add:liftingvm_valid_def)

lemma liftingvm_valid_unfold1 :
  assumes H : "liftingvm_valid l"
  shows "liftingv_valid l" using H 
  by (auto simp add:liftingvm_valid_def)

lemma liftingvm_valid_unfold2 :
  assumes H : "liftingvm_valid l"
  assumes H1 : "b' \<in> LMgS l s b"
  assumes H2 : "a = LOut l s b"
  assumes H3 : "a' = LOut l s b'"
  shows "LOut l s [^ b, b' ^] = [^ a, a' ^]"
  using H H1 H2 H3
  by(auto simp add:liftingvm_valid_def)
*)
...
(* problem: we would need predicate to hold on base
we could do that, but maybe we want to express a property
that more directly relates to the lifting
(although this should be true of the lifting...?
*)

(* need to somehow generalize this more -
   we are seeming to depend too much on "extra information"
   that we shouldn't care about (morally P should be talking about
   "just the inner data" *)

(* i think we need another hypothesis about f "preserving"
   the presence in OutS *)
...
proof(rule allI; rule impI)
  ...
  have Eq1 : "LOut l2 (l1 syn') (LNew l2 x1 (LOut l2 x1 x2')) = LOut l2 (l1 syn') x2'"
    using Hsyn' lifting_valid_unfold1[ OF Hv] by auto
...
"LOut l2 (l1 syn') (LIn l2 x1 (LOut l2 x1 (f' (SOME x. l1 x = x1) (LIn l2 x1 (LOut l2 x1 x2') (LBase l2)))) (LIn l2 x1 (LOut l2 x1 x2') (LBase l2))) =
  (LOut l2 x1 (f' (SOME x. l1 x = x1) (LIn l2 x1 (LOut l2 x1 x2') (LBase l2))))"
    using lifting_valid_unfold2[OF Hv, of "l1 syn'"] Hsyn'
    by(auto)
...
"LOut l2 (l1 (SOME x. l1 x = x1)) (f' (SOME x. l1 x = x1) (LIn l2 x1 (LOut l2 x1 x2') (LBase l2))) = LOut l2 (l1 syn') (f' syn' x2')"
  proof(rule can_unmap_unfold[OF Hunmap Syn_eq])
    show "LOut l2 (l1 (SOME x. l1 x = x1)) (LIn l2 x1 (LOut l2 x1 x2') (LBase l2)) = LOut l2 (l1 syn') x2'"
      using Syn Hsyn' lifting_valid_unfold2[OF Hv, of "l1 (SOME x. l1 x = x1)"]
      by(simp)
  qed
...
lemma lift_lower' :
  ...
  assumes H:
    "l_unpred_step_s l1 l2 (l_pred_step_s l1 l2 P) x1 x2
                           (l_unmap_s l1 l2 (l_map_s l1 l2 f) x1 x2)"
  shows "P x1 x2 (f x1 x2)" using Hv H
  unfolding l_unpred_step_s_def l_pred_step_s_def
  apply(clarsimp; auto)


(* relate f and unmap (map f) *)
