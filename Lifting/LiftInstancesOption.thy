...
definition option_dfl_pl ::
  "'a \<Rightarrow> ('x, 'a, 'b, 'z) plifting_scheme \<Rightarrow> ('x, 'a, 'b option) plifting" where
"option_dfl_pl dfl t =
  \<lparr> LUpd = (\<lambda> s a b . 
            (case b of
              Some b' \<Rightarrow> Some (LUpd t s a b')
              | None \<Rightarrow> Some (LNew t s a)))
  , LOut = (\<lambda> s b . (case b of Some b' \<Rightarrow> LOut t s b'
                      | None \<Rightarrow> Some dfl))
  , LBase = (\<lambda> s . None)\<rparr>"

definition option_dfl_l ::
  "'a => ('x, 'a, 'b, 'z) lifting_scheme \<Rightarrow> ('x, 'a, 'b option) lifting" where
"option_dfl_l dfl t =
  plifting.extend (option_dfl_pl dfl t)
    \<lparr> LPost = (\<lambda> s b . (case b of (Some b') \<Rightarrow> Some (LPost t s b')
                                  | None \<Rightarrow> None)) \<rparr>"

definition option_dfl_pv ::
  "'a \<Rightarrow> ('x, 'a, 'b, 'z) pliftingv_scheme \<Rightarrow>  ('x, 'a, 'b option) pliftingv" where
"option_dfl_pv dfl v  =
  \<lparr> LOutS = (\<lambda> s . Some ` (LOutS v s)) \<rparr>"

lemma option_dfl_pl_valid :
  assumes H : "plifting_valid t"
  shows "plifting_valid (option_dfl_pl dfl t)"
proof(rule plifting_valid_intro)
  fix s a b
  show "LOut (option_dfl_pl dfl t) s (LUpd (option_dfl_pl dfl t) s a b) = Some a"
    using plifting_valid_unfold1[OF H]
    by(auto simp add:option_dfl_pl_def LNew_def split:option.splits)
next
  fix s a a' b
  show "LUpd (option_dfl_pl dfl t) s a (LUpd (option_dfl_pl dfl t) s a' b) = LUpd (option_dfl_pl dfl t) s a b"
    using plifting_valid_unfold2[OF H]
    by(auto simp add:option_dfl_pl_def LNew_def split:option.splits)
qed

lemma option_dfl_pl_pv_valid : 
  "plifting_pv_valid (t :: ('x, 'a, 'b, 'z1) plifting_scheme) (v :: ('x, 'a, 'b, 'z2) pliftingv_scheme) \<Longrightarrow> 
   plifting_pv_valid (option_dfl_pl dfl t) (option_dfl_pv dfl v)"
proof(rule plifting_pv_valid_intro)
  assume H : "plifting_pv_valid t v"
  show "plifting_valid (option_dfl_pl dfl t)" using option_dfl_pl_valid[OF plifting_pv_valid_unfold1[OF H]]
    by auto
next
  fix s a b
  assume H : "plifting_pv_valid t v"
  show "LUpd (option_dfl_pl dfl t) s a b \<in> LOutS (option_dfl_pv dfl v) s"
    using plifting_pv_valid_unfold2[OF H]
    by(auto simp add:option_dfl_pl_def option_dfl_pv_def LNew_def split:option.splits)
next
  fix s b
  assume H : "plifting_pv_valid t v"
  assume H' : "b \<in> LOutS (option_dfl_pv dfl v) s"
  show "\<exists>out. LOut (option_dfl_pl dfl t) s b = Some out \<and>
                 b = LUpd (option_dfl_pl dfl t) s out b"
    using plifting_pv_valid_unfold3[OF H] H'
    by(auto simp add:option_dfl_pl_def option_dfl_pv_def)
qed

lemma option_dfl_l_pv_valid :
  assumes H : "lifting_pv_valid t v"
  shows "lifting_pv_valid (option_dfl_l dfl t) (option_dfl_pv dfl v)"
proof(rule lifting_pv_valid_intro)
  show "plifting_pv_valid (option_dfl_l dfl t) (option_dfl_pv dfl v)"
    unfolding option_dfl_l_def
  proof(rule plifting_pv_valid_cast)
    show "plifting_pv_valid (option_dfl_pl dfl t) (option_dfl_pv dfl v)"
      using option_dfl_pl_pv_valid[OF lifting_pv_valid_unfold1[OF H]]
      by(auto)
  qed
next
  fix s a b
  assume H1 : "b \<in> LOutS (option_dfl_pv dfl v) s"
  show "LPost (option_dfl_l dfl t) s b \<in> LOutS (option_dfl_pv dfl v) s" unfolding option_l_def
    using lifting_pv_valid_unfold2[OF H] H1
    by(auto simp add: option_dfl_l_def option_dfl_pv_def plifting.defs)
next
  fix s a b
  assume H1 : "b \<in> LOutS (option_dfl_pv dfl v) s "
  show "LOut (option_dfl_l dfl t) s (LPost (option_dfl_l dfl t) s b) = LOut (option_dfl_l dfl t) s b"
    using lifting_pv_valid_unfold3[OF H] H1
    by(auto simp add: option_dfl_l_def option_dfl_pl_def option_dfl_pv_def plifting.defs)
qed
...
(* TODO: this is now forcing default values - perhaps a bad idea...*)
definition prod_pl ::
