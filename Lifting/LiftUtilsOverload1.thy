theory LiftUtilsOverload1
  imports "LiftUtils" "LiftInstances1" "HOL-Library.Adhoc_Overloading"
begin

(* new idea:
   - define an overloaded constant assigning reified liftings to each type (rtype?)
   - maybe we just overload on output type. or do we need to overload at all?
   - problem is that reified type can't be in typeclass
      (wait actually i think they can? we should be able to know that 
      (denote r :: 'a :: reiden)
*)
