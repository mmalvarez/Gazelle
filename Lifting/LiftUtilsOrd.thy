...
"('x, 'a, 'b :: Pord) liftingv \<Rightarrow> ('x \<Rightarrow> 'a \<Rightarrow> 'a) \<Rightarrow> bool" where
"respects_InS l f =
  (\<forall> x a . a \<in> LInS l x \<longrightarrow> f x a \<in> LInS l x)"

definition respects_OutS ::
"('x, 'a, 'b :: Pord) liftingv \<Rightarrow> ('x \<Rightarrow> 'b \<Rightarrow> 'b) \<Rightarrow> bool" where
"respects_OutS l f =
...
proof(rule lower_pred_step_sI)
  ...
  have Eq4 : " LOut l2 (l1 syn') (f' syn' x2') =
    ...
    using lifting_validD1[OF Hv] leq_refl unfolding Hsyn by(auto)
...
proof(rule can_lower_map_sI)
  ...
  show "LOut l2 (l1 b1_1) (lift_map_s l1 l2 f b1_1 b2_1) =
    ...
    unfolding lift_map_s_def using Hsem lifting_validD1[OF Hv]
