...
record ('syn, 'a, 'b) lifting =
  ...
  LNew :: "('syn \<Rightarrow> 'a \<Rightarrow> 'b)"
...
"lifting_valid l =
  ((\<forall> s a b . LOut l s (LIn l s a b) = a) \<and>
   (\<forall> s a . LOut l s (LNew l s a) = a))"

lemma lifting_valid_intro :
  assumes H1 : "\<And> s a b . LOut l s (LIn l s a b) = a"
  assumes H2 : "\<And> s a . LOut l s (LNew l s a) = a"
...
"liftingv_valid l =
  ((lifting_valid l) \<and>
   ...
   (\<forall> s a . LNew l s a \<in> LOutS l s) \<and>
   (\<forall> s a b . LNew l s a <[ LIn l s a b) \<and>
   (\<forall> s b b' .
      b' <[ b \<longrightarrow>
      b \<in> LOutS l s \<longrightarrow>
      b' <[ LIn l s (LOut l s b) b') \<and>
   (\<forall> s a a' a'' b . 
...
lemma l_pred_step_s_map_s :
  ...
  using Hsyn Hsem HP lifting_valid_unfold1[OF Hv] 
