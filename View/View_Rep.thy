...
fun to_r' :: "'b \<Rightarrow> ('x + ('z) + ('a * 'y))" where
"to_r' b =
  (case prj b of
      Inr _ \<Rightarrow> (case prj (inj (undefined, b)) of
                      Inr _ \<Rightarrow> Inl (Xabs b)
                      | Inl _ \<Rightarrow> Inr (Inl (Zabs (\<lambda> a . inj(a, b), b))))
    | Inl a \<Rightarrow> Inr (Inr (a, Yabs (\<lambda> a. inj (a, b)))))"
...
abbreviation rep' :: "('b, ('x + ('z) + ('a * 'y))) iso_parm" where
"rep' \<equiv> (to_r', to_l')"
...
sublocale View_Rep_Complete \<subseteq> View_Rep fzy' rep'
  apply(unfold_locales)
  ...
  apply(auto split:sum.splits)
     ...
     apply(case_tac b, clarsimp)
        apply(cut_tac z = a in Zrep_spec) apply(simp add:Z_def)
  apply(simp split:prod.splits)

     apply(clarsimp)
       apply(cut_tac y = ba in Yrep_spec) apply(simp add:Y_def) apply(auto)
