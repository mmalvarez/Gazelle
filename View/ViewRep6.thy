theory ViewRep6 imports View "../Isos/Iso" "../Lens/Lens"
...
locale View_Rep' =
  fixes lensy :: "('y, 'yu) lens_parms"
  fixes isobig :: "('a, ('x + 'y + 'b + ('b * 'yu))) iso_parm"
  

locale View_Rep =
  View_Rep' +
  Ly : Lens_Spec "lensy :: ('y, 'yu) lens_parms" +
  Ibig : Iso_Spec "isobig :: ('a, ('x + 'y + 'b + ('b * 'yu))) iso_parm"
...
"prj' a =
  (case Ibig.to_r a of
    ...
    | Inr (Inr ((Inl b))) \<Rightarrow> Inl b
    | Inr (Inr (Inr (b, _))) \<Rightarrow> Inl b)"

fun inj' :: "'b * 'a \<Rightarrow> 'a" where
"inj' (b, a) =
  (case Ibig.to_r a of
    Inl _ \<Rightarrow> a
    | Inr (Inl (y)) \<Rightarrow> Ibig.to_l (Inr (Inr (Inr (b, prj 
    | Inr (Inr ((Inl b'))) \<Rightarrow> Ibig.to_l (Inr (Inr (Inl b)))
    | Inr (Inr (Inr (Inl (b', yu)))) \<Rightarrow> Ibig.to_l (Inr (Inr (Inr (Inl (b, yu))))))"
