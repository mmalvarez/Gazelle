theory View_Rep2A imports View "../Isos/Iso"
...
sublocale View_Rep_Complete \<subseteq> View_Rep rep'
  ...
  apply(case_tac "local.prj (local.inj (a, l))") apply(clarsimp)
  ...
  apply(simp add:PrjInj1)
     ...
     apply(cut_tac b = l in Yabs_inverse) apply(simp add: Y_def) apply(auto)
      apply(drule_tac PrjInj2) apply(clarsimp)
...
*)
  ...
  apply(case_tac r) apply(clarsimp)
      ...
      apply(case_tac b) apply(clarsimp)
      apply(cut_tac y = a in Yrep_spec) apply(simp add:Y_def) apply(clarsimp) apply(safe)
