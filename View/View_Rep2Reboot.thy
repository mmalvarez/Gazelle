theory View_Rep2Reboot imports View "../Isos/Iso"
...
locale View_Rep' =
  fixes repp :: "('a, ('x + ('b \<Rightarrow> 'a) + ('b * ('b \<Rightarrow> 'a)))) iso_parm"

locale View_Rep =
  View_Rep' +
  I : Iso_Spec "repp :: ('a, ('x + ('b \<Rightarrow> 'a) + ('b * ('b \<Rightarrow> 'a)))) iso_parm"
...
"inj' (b, a) =
  (case I.to_r a of
    Inl _ \<Rightarrow> a
    | Inr (Inl (z)) \<Rightarrow> I.to_l (Inr (Inr (b, z)))
...
in Zrep_inverse) apply(auto)
  ...
  apply(subgoal_tac
"Zrep (Zabs (\<lambda>aa. local.inj (aa, Yrep a))) = Zrep ba")
      defer
  apply(cut_tac f = 
"(\<lambda>aa. local.inj (aa, local.inj(a', Yrep a)))" in Zabs_inverse)
       apply(simp add:Z_def)
  apply(rule_tac x = "local.inj(a', Yrep a)" in exI) apply(auto)
