theory View_Rep4 imports View "../Isos/Iso"
...
locale View_Rep' =
  fixes repp :: "('a, ('x + ('y * ('b \<Rightarrow> 'z)) + ('b * 'z))) iso_parm"

locale View_Rep =
  View_Rep' +
  I : Iso_Spec "repp :: ('a, ('x + ('y * ('b \<Rightarrow> 'z)) + ('b * 'z))) iso_parm"
...
"inj' (b, a) =
  (case I.to_r a of
    Inl _ \<Rightarrow> a
    | Inr (Inl (y, z)) \<Rightarrow> I.to_l (Inr (Inr (b, z b)))
...
fun to_r' :: "'b \<Rightarrow> ('x + ('y * ('b \<Rightarrow> 'z)) + ('a * ('b \<Rightarrow> 'z)))" where
"to_r' b =
  (case prj b of
      Inr _ \<Rightarrow> (case prj (inj (undefined, b)) of
                      Inr _ \<Rightarrow> Inl (Xabs b)
                      | Inl _ \<Rightarrow> Inr (Inl (Yabs b, (\<lambda> b . Zabs (\<lambda> a . inj (a, b))))))
