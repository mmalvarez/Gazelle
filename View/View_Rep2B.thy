theory View_Rep2B imports View "../Isos/Iso"
...
locale View_Rep' =
  fixes repp :: "('a, ('x + ('y * 'z) + ('b * ('z1 + 'z2)))) iso_parm"

locale View_Rep =
  View_Rep' +
  I : Iso_Spec "repp :: ('a, ('x + ('y * 'z1) + ('b * ('z1 + 'z2)))) iso_parm"
...
"inj' (b, a) =
  (case I.to_r a of
    Inl _ \<Rightarrow> a
    | Inr (Inl (y, z1)) \<Rightarrow> I.to_l (Inr (Inr (b, (Inl z1))))
    | Inr (Inr (b', z12)) \<Rightarrow> I.to_l (Inr (Inr (b, z12))))"
...
definition X :: "'b set" where
  "X = { (b :: 'b) . \<forall> a . prj (inj (a, b)) = Inr b }"


definition Z1 :: "('a \<Rightarrow> 'b) set" where
  "Z1 = {f :: ('a \<Rightarrow> 'b) .
          (\<exists> b . f = (\<lambda> a . inj (a, b)) \<and>
          (prj b = Inr b)) }"

(*
definition Z2 :: "('a \<Rightarrow> 'b) set" where
  "Z2 = {f :: ('a \<Rightarrow> 'b) .
          (\<exists> b . f = (\<lambda> a . inj (a, b))) \<and>
          (\<forall> b . f = (\<lambda> a . inj (a, b)) \<longrightarrow>
          (\<exists> a' . prj b = Inl a')) \<and>
  (\<forall> a b . f a = inj (a, b) \<longrightarrow>
            (\<exists> a' . prj b = Inl a')) }"
*)
...
locale View_Rep_Complete = View_Rep_Complete' +
  ...
  fixes Z1rep :: "'z1 \<Rightarrow> ('a \<Rightarrow> 'b)"
  fixes Z1abs :: "('a \<Rightarrow> 'b) \<Rightarrow> 'z1"
  fixes Z2rep :: "'z2 \<Rightarrow> ('a \<Rightarrow> 'b)"
  fixes Z2abs :: "('a \<Rightarrow> 'b) \<Rightarrow> 'z2"

  assumes Xrep_spec : "\<And> x . Xrep x \<in> X"
  assumes Yrep_spec : "\<And> y . Yrep y \<in> Y"
  assumes Z1rep_spec : "\<And> z . Z1rep z \<in> Z1"
  assumes Z2rep_spec : "\<And> z . Z2rep z \<in> Z2"


assumes Xrep_inverse : "\<And> x . Xabs (Xrep x) = x"
assumes Yrep_inverse : "\<And> y . Yabs (Yrep y) = y"
assumes Z1rep_inverse : "\<And> z . Z1abs (Z1rep z) = z"
assumes Z2rep_inverse : "\<And> z . Z2abs (Z2rep z) = z"
...
assumes Z1abs_inverse : "\<And> f . f \<in> Z1 \<Longrightarrow> Z1rep (Z1abs f) = f"
assumes Z2abs_inverse : "\<And> f . f \<in> Z2 \<Longrightarrow> Z2rep (Z2abs f) = f"
...
(*
fun to_r' :: "'b \<Rightarrow> ('x + ('y * 'z1) + ('a * ('z1 + 'z2)))" where
"to_r' b =
  (case prj b of
      Inr _ \<Rightarrow> (case prj (inj (undefined, b)) of
                      Inr _ \<Rightarrow> Inl (Xabs b)
                      | Inl _ \<Rightarrow> Inr (Inl (Yabs b, Z1abs (\<lambda> a . inj (a, b)))))
    | Inl a \<Rightarrow> 
      (if {(b' :: 'b) . \<exists> (a :: 'a) . inj (a, b') = b \<and> prj b' = Inr b'} = {} then
          Inr (Inr (a, Inr (Z2abs (\<lambda> a . inj (a, b)))))
       else
          Inr (Inr (a, Inl (Z1abs (\<lambda> a . inj (a, b)))))))"
*)

fun to_r' :: "'b \<Rightarrow> ('x + ('y * 'z1) + ('a * ('z1 + 'z2)))" where
"to_r' b =
  (case prj (inj (undefined, b)) of
                      Inr _ \<Rightarrow> Inl (Xabs b)
                      | Inl _ \<Rightarrow> 
      (case prj b of
        Inr _ \<Rightarrow>   Inr (Inl (Yabs b, Z1abs (\<lambda> a . inj (a, b))))
      | Inl a \<Rightarrow> 
        (if {(b' :: 'b) . (\<lambda> a . inj (a, b)) = (\<lambda> a . inj (a, b')) \<and> prj b' = Inr b'} = {} then
          Inr (Inr (a, Inr (Z2abs (\<lambda> a . inj (a, b)))))
         else
          Inr (Inr (a, Inl (Z1abs (\<lambda> a . inj (a, b))))))))"
...
fun to_l' :: "('x + ('y * 'z1) + ('a * ('z1 + 'z2))) \<Rightarrow> 'b" where
"to_l' (Inl x) = Xrep x"
| "to_l' (Inr (Inl (y, z1))) = 
    Yrep y" (* this one is bad. ? *)
| "to_l' (Inr (Inr (a, Inl z1))) =
    Z1rep z1 a"
| "to_l' (Inr (Inr (a, Inr z2))) =
    Z2rep z2 a"
...
abbreviation rep' :: "('b, ('x + ('y * 'z1) + ('a * ('z1 + 'z2)))) iso_parm" where
"rep' \<equiv> (to_r', to_l')"
...
sublocale View_Rep_Complete \<subseteq> View_Rep rep'
  apply(unfold_locales)
   apply(simp split:sum.splits) apply(safe)
    apply(cut_tac f = "(\<lambda>a. local.inj (a, l))" in "Z2abs_inverse")
         apply(simp add:Z2_def) apply(auto)
          
        apply(simp add:PrjInj1)

       apply(cut_tac b = l in Xabs_inverse) apply(auto simp add:X_def)
  apply(drule_tac d' = a in InjPrj2) apply(simp)

      apply(cut_tac f = "(\<lambda>a. local.inj (a, x))" in Z1abs_inverse) apply(simp add:Z1_def)
       apply(rule_tac x = x in exI) apply(simp)
      apply(simp)
      apply(drule_tac x = x1 in fun_cong) apply(frule_tac r = l in PrjInj1) apply(simp)


         apply(cut_tac b = l in Xabs_inverse) apply(auto simp add:X_def)
  apply(drule_tac d' = a in InjPrj2) apply(simp)

    apply(cut_tac b = l in Yabs_inverse) apply(auto simp add:Y_def)
     apply(drule_tac PrjInj2) apply(simp)
  ...
  apply(case_tac ba) apply(clarsimp)
         ...
         apply(cut_tac z = aa in Z1rep_spec) apply(auto simp add:Z1_def)
         apply(drule_tac InjPrj1) apply(simp)

         apply(cut_tac z = b in Z2rep_spec) apply(auto simp add:Z2_def)
        apply(drule_tac InjPrj1) apply(simp)
  ...
  apply(case_tac ba) apply(clarsimp)
       ...
       apply(cut_tac z = b in Z2rep_spec) apply(auto simp add:Z2_def)
       apply(simp add: InjInj)
       apply(cut_tac z = b in Z2rep_inverse) apply(simp)

       apply(case_tac r) apply(clarsimp)
       apply(cut_tac x = a in Xrep_inverse) apply(auto simp add:X_def)
      ...
      apply(case_tac ba) apply(clarsimp)
        ...
        apply(cut_tac z = aa in Z1rep_spec) apply(auto simp add:Z1_def)
       apply(drule_tac x = b in spec) apply(clarsimp) apply(simp add:InjInj)
     ...
     apply(clarsimp)
      ...
      apply(simp add:InjInj)
      apply(cut_tac z = " aa" in Z1rep_inverse) apply(simp)

       apply(cut_tac z = b in Z2rep_spec) apply(simp add:Z2_def)
  apply(auto)
     apply(simp add: InjInj)
      apply(frule_tac r = ba in PrjInj1) apply(simp)
  ...
  apply(case_tac b) apply(clarsimp)
         apply(cut_tac y = a in Yrep_spec) apply(simp add:Y_def)
        apply(clarsimp) apply(case_tac ba) apply(clarsimp)

  apply(cut_tac z = aa in Z1rep_spec) apply(simp add: Z1_def) apply(clarsimp)
...
*)
  ...
  apply(case_tac r) apply(clarsimp)
      ...
      apply(case_tac b) apply(clarsimp)
          apply(cut_tac y = a in Yrep_spec) apply(simp add:Y_def)
          apply(cut_tac z = ba in Zrep_spec) apply(clarsimp)
  ...
  apply(clarsimp) 
         ...
         apply(auto)
          ...
          apply(cut_tac z = ba in Zrep_spec) apply(simp add:Z_def)
         apply(auto) apply(simp add:InjInj) apply(cut_tac z = ba in Zrep_inverse) apply(simp)
  ...
  apply(case_tac r) apply(clarsimp)
      ...
      apply(case_tac b) apply(clarsimp)
         apply(auto)
          apply(cut_tac y = a in Yrep_spec) apply(simp add:Y_def) 
          apply(cut_tac z = ba in Zrep_spec) apply(clarsimp)
          apply(drule_tac x = "Zrep ba" in spec) apply(clarsimp)

          apply(subgoal_tac "Yrep (Yabs (Pair (fst (Yrep a (Zrep ba))))) = Yrep a")
           defer
  apply(cut_tac y = "(Yabs (Pair (fst (Yrep a (Zrep ba)))))" in Yrep_spec)
  apply(simp add: Y_def) apply (drule_tac x = "Zrep ba" in spec)
  apply(clarsimp)

  apply(drule_tac d = x1a in PrjInj1)
      apply(cut_tac z = ba in Zrep_spec) apply(simp add:Z_def)
     apply(cut_tac z = ba in Zrep_spec) apply(simp add:Z_def)
     apply(clarsimp)
     apply(simp add:InjInj)
     apply(cut_tac z = ba in Zrep_inverse) apply(clarsimp)

  apply(case_tac r) apply(clarsimp)
      apply(cut_tac x = a in Xrep_spec) apply(simp add:X_def) apply(clarsimp)

    apply(case_tac b) apply(clarsimp) apply(safe)
  apply(rule_tac Yrep_inverse)
     apply(cut_tac y = a in Yrep_spec) apply(simp add:Y_def) apply(clarsimp)
     (* Rep_iszero_inject *)
  apply(subgoal_tac "Zrep (Zabs (\<lambda>aa. local.inj (aa, Yrep a))) = Zrep ba")
         defer
         apply(cut_tac z = ba in Zrep_spec) apply(simp add:Z_def) apply(clarsimp)
  apply(subgoal_tac 
     "(\<lambda>aa. local.inj (aa, Yrep a)) = (\<lambda>a. local.inj (a, b))")defer
