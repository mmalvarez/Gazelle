...
(* DEPRECATED EXPERIMENT, see Syntax_Utils.thy
for current version of these ideas *)

(* 

right now this is a clone of an old version of Alt_Sym

the original idea was to change our normal form to ('a * ('x1 * ... * 'xn) 'c)

but now we have changed the problem and need to reassociate elsewhere
...
fun sym_elle_cases_gen' ::
"('i, 'lix, 'ljx, 'ljix, 'xb1, 'xb2, 'xa) sym_elle \<Rightarrow> 
 ('i, 'lix, 'ljx, 'ljix, 'xb1, 'xb2, 'xa, 'o) sym_elle_disc'2 \<Rightarrow> 'o" where
"sym_elle_cases_gen' s d = sym_i_cases_gen' s (tmap3t dmerge3 d)"
