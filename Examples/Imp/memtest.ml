structure MemTest : sig
  ...
  type 'a md_triv
  val test2 :
    (int md_triv option) md_prio * (string, (int md_triv option) md_prio) oalist
end = struct
...
fun test_state (A1_, A2_, A3_) = (swr Zero_int, test_store (A1_, A2_, A3_));
...
fun option_dfl_pl dfl t =
  Plifting_ext
    ((fn s => fn a => fn b =>
       (case b of NONE => SOME (lNew t s a) | SOME ba => SOME (lUpd t s a ba))),
      (fn s => fn a => (case a of NONE => SOME dfl | SOME aa => lOut t s aa)),
      (fn _ => NONE), ());
...
fun option_dfl_l dfl t =
  extend (option_dfl_pl dfl t)
    (Lifting_ext
      ((fn s => fn a =>
...
fun option_pl t =
  Plifting_ext
    ((fn s => fn a => fn b =>
       (case b of NONE => SOME (lNew t s a) | SOME ba => SOME (lUpd t s a ba))),
      (fn s => fn a => (case a of NONE => NONE | SOME aa => lOut t s aa)),
      (fn _ => NONE), ());
...
fun prod_pl t1 t2 =
  Plifting_ext
    ((fn s => fn a => fn b => let
      ...
      (fn s => fn (b1, b2) =>
        (case lOut t1 s b1 of NONE => NONE
          | SOME b1o =>
            (case lOut t2 s b2 of NONE => NONE | SOME b2o => SOME (b1o, b2o)))),
...
val id_pl : ('a, 'b, 'b, unit) plifting_ext =
  Plifting_ext
    ((fn _ => fn a => fn _ => a), (fn _ => SOME),
      (fn _ => (raise Fail "undefined")), ());

fun pl_map_s la l sem syn st =
  (case lOut l (la syn) st of NONE => st
  ...
  l_map_s id
    (prod_l
      ((prio_l_inc o option_l o triv_l)
        (extend id_pl (Lifting_ext ((fn _ => fn a => a), ()))))
      (oalist_l (equal_literal, linorder_literal) mem_keys
        ((prio_l_inc o option_dfl_l Zero_int o triv_l)
          (extend id_pl (Lifting_ext ((fn _ => fn a => a), ()))))))
    mem0_sem x;
